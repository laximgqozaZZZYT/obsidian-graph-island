...
describe("resolveAxisValues", () => {
	...
	it("property: numeric values parsed correctly", () => {
		const nodes2 = [makeNode("a", { score: 10 }), makeNode("b", { score: 20 }), makeNode("c", { score: 5 })];
	...
	describe("shape-fill", () => {
		...
		it("square: axis 2 returns y coordinates", () => {
			...
			for (const [id] of raw25) {
				if (Math.abs(tx.get(id)! - ty.get(id)!) > 0.001) {
					anyDiff = true;
					break;
				}
			}
...
describe("resolveAxisCategories", () => {
	it("returns sorted unique string categories for property source via meta", () => {
		...
		const cats = resolveAxisCategories(nodes, { kind: "property", key: "node_type" }, baseCtx());
		expect(cats).toBeDefined();
		expect(cats).toEqual(["alpha", "beta"]);
	});

	it("returns undefined for numeric field values", () => {
		const nodes = [makeNode("a", { score: 10 }), makeNode("b", { score: 20 })];
		const cats = resolveAxisCategories(nodes, { kind: "field", field: "score" }, baseCtx());
	...
	it("returns empty-string categories for nodes missing the field", () => {
		...
		const cats = resolveAxisCategories(nodes, { kind: "field", field: "nonexistent" }, baseCtx());
	...
	it("handles Japanese category names via property source", () => {
		...
		const cats = resolveAxisCategories(nodes, { kind: "property", key: "type" }, baseCtx());
		expect(cats).toBeDefined();
	...
	it("returns categories for property source via meta", () => {
		const nodes = [makeNode("a", { node_type: "character" }), makeNode("b", { node_type: "location" })];
		const cats = resolveAxisCategories(nodes, { kind: "property", key: "node_type" }, baseCtx());
		expect(cats).toBeDefined();
...
describe("resolveAxisValues edge cases (cycle152)", () => {
	...
	it("metric:degree for nodes not in degree map defaults to 0", () => {
		...
		const vals = resolveAxisValues([makeNode("a"), makeNode("b")], { kind: "metric", metric: "degree" }, ctx2);
		expect(vals.get("a")).toBe(0);
		expect(vals.get("b")).toBe(0);
	});
...
describe("applyTransform edge cases (cycle152)", () => {
	...
	it("expression with N variable uses constants", () => {
		...
		const t = applyTransform(raw, { kind: "expression", expr: "t * N", scale: 1 }, 10, undefined, { N: 100 });
