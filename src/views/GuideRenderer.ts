...
export class GuideRenderer {
	...
	drawCoordinateGuide(
		...
		if (guide.gridInfo) {
			...
			this.drawAxisTitles(
				cx,
				cy,
				guide.gridInfo.axis1Shape,
				guide.gridInfo.axis2Shape,
				bounds,
				worldScale,
				isDark,
				guide.axis1Label,
	...
	drawTimelineAxis(
		...
		if (
			rt.timelineAxisShowLabels &&
			this.host.getPanel().showTimelineTickLabels !== false &&
			guide.ticks.length > 0
		) {
	...
	private drawGridTicks(
		...
		axis1Lines: ResolvedGridLine[],
		axis2Lines: ResolvedGridLine[],
		axis1Shape: { kind: string },
		axis2Shape: { kind: string },
		bounds: { xMin: number; yMin: number; xMax: number; yMax: number; maxR?: number },
		lineW: number,
		color: number,
		worldScale: number,
	...
	private drawGridLabels(
		...
		const fontSize = Math.max(
			rt.gridLabelFontSizeMin,
			Math.min(rt.gridLabelFontSizeMax, rt.gridLabelFontSizeBase / worldScale),
		);
