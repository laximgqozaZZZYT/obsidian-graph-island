...
export class LabelManager {
	...
	private _applyDiversityAndCap(
		...
		const zoom = this.host.getWorldScale();
		// Map-style labeling: no maxVisible cap on desktop. Mobile caps to 50 for perf.
		...
		this._applyZoomEmphasis(candidates, zoom);
	}

	/** Apply per-candidate visibility based on hover/super bypass and maxVisible cap.
	 *  Returns the number of regular (non-bypass) labels marked visible. */
	private _applyVisibilityCap(
		...
		const visibleAlpha = Math.max(rt.labelAlphaMin ?? 0.7, baseOpacity);
		let visCount = 0;
	...
	/** Hide a label that exceeded the maxVisible cap, including any tag/sub labels. */
	private _hideCappedLabel(pn: PixiNode): void {
		pn.label!.visible = false;
		pn.label!.alpha = 0;
		pn.labelWasVisible = false;
		if (pn.tagLabel) pn.tagLabel.visible = false;
	 ...
	 *  candidates is already priority-desc sorted, so we re-enable hidden
	 *  candidates from the top until the floor is met. */
