...
describe("renderGraphStats", () => {
	it("hides element when showGraphStats is false", () => {
		...
		renderGraphStats(
			el as any,
			{ nodes: [], edges: [] },
			defaultPanel({ showGraphStats: false }),
			createMockHost(),
		);
	...
	it("renders top hubs section for high-degree nodes", () => {
		...
		const edges = [mkEdge("hub1", "leaf"), mkEdge("hub2", "leaf"), mkEdge("hub1", "hub2")];
	...
	it("renders edge type distribution", () => {
		...
		const edges = [mkEdge("a", "b", "link"), mkEdge("a", "c", "link"), mkEdge("b", "c", "inheritance")];
		const degrees = new Map([
			["a", 2],
			["b", 2],
			["c", 2],
	...
	it("renders structure questions when enabled", () => {
		...
		renderGraphStats(el as any, { nodes, edges }, defaultPanel({ showStructureQuestions: true }), host);
