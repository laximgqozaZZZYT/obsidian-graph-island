...
export function applySunburstLayout(
	...
	const effectiveRoot =
		matched < graphFilePaths.size * COVERAGE_THRESHOLD || countDirectChildren(root) <= 1
	...
	const nodes = gd.nodes.map((n) => {
		...
		const angle = unmatchedTotal > 0 ? (unmatchedIdx++ / unmatchedTotal) * 2 * Math.PI : 0;
		return {
			...n,
...
export function computeSunburstArcs(root: SunburstData, width: number, height: number): SunburstArc[] {
	const radius = Math.min(width, height) / 2;
	const arcs: SunburstArc[] = [];

	assignValues(root);

	function traverse(node: SunburstData, depth: number, startAngle: number, endAngle: number) {
