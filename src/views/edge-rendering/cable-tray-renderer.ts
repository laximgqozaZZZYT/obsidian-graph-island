/**
 * Cable-tray drawing routines extracted from EdgeRenderer.
 *
 * These functions render the 3-layer wiring model (Trunk → Cable → Wire)
 * into a CanvasGraphics. They are pure with respect to module state — all
 * needed inputs (cables/trunks, config, lane info) are passed as arguments.
 *
 * Layout / pure computation lives in CableTrayRenderer.ts; this file owns
 * only the draw-side logic.
 */
...
import { resolveEdgeColor } from "../EdgeRenderer";
 ...
 * Uses existing drawSmoothPath for all rendering.
 */
