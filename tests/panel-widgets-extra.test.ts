...
describe("serializeGroupByRules (extra)", () => {
	...
	it("serializes with default AND operator", () => {
		const result = serializeGroupByRules([{ field: "tag" }, { field: "category" }]);
		expect(result).toBe("tag AND category");
	});

	it("serializes with explicit OR operator", () => {
		const result = serializeGroupByRules([{ field: "tag", op: "OR" }, { field: "category" }]);
...
describe("deriveClusterRulesFromGroupBy (extra)", () => {
	it("filters empty field names", () => {
		const result = deriveClusterRulesFromGroupBy([{ field: "tag" }, { field: "  " }, { field: "category" }]);
	...
	it("respects recursive flag", () => {
		const result = deriveClusterRulesFromGroupBy([{ field: "folder", recursive: true }]);
