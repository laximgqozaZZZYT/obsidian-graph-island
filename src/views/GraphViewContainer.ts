...
import {
	...
	buildTransitionData,
	computeTimelineFit,
	computeTimelineArrowNavigation,
...
export class GraphViewContainer extends ItemView implements InteractionHost, RenderHost, LayoutHost {
	...
	private _handleTimelineArrowKey(key: string): void {
		...
		const target = computeTimelineArrowNavigation(bars, this.highlightedNodeId, key as TimelineArrowKey);
		if (!target) return;
	...
	private _buildHoverHighlightSet(hId: string | null): Set<string> {
		...
		return capHoverLabelsBySet(result, hId, maxNeighborLabels, this.degrees);
	}
