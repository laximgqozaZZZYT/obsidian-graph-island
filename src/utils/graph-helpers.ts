...
/** Count how many non-tag nodes carry each tag (specificity = lower count). */
...
/** Pick the tag with the smallest member count from a non-empty list. */
...
/** Collect symmetric pairs of tag IDs joined by inheritance/aggregation edges. */
function collectTagRelationshipPairs(edges: readonly GraphEdge[]): Set<string> {
...
export function buildTagMembership(
	...
	const tagRelPairs = collectTagRelationshipPairs(edges);
