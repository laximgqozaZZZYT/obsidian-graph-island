...
describe("buildAdjFiltered", () => {
	...
	const edges = [mkEdge("a", "b", "link"), mkEdge("b", "c", "semantic"), mkEdge("a", "c", "tag")];
...
describe("parseGroupByFields", () => {
	...
	it("AND strips operator", () => expect(parseGroupByFields("folder AND tag")).toEqual(["folder", "tag"]));
	it("OR strips operator", () =>
		expect(parseGroupByFields("folder:? OR category:?")).toEqual(["folder", "category"]));
	it("XOR strips operator", () => expect(parseGroupByFields("tag XOR louvain")).toEqual(["tag", "louvain"]));
	it("case-insensitive operators", () => expect(parseGroupByFields("folder or tag")).toEqual(["folder", "tag"]));
...
describe("computeAutoFitTransform", () => {
	const mkFitNodes = (coords: [number, number][]) => coords.map(([x, y]) => ({ x, y, r: 10 }));
	...
	it("2000 skewed nodes: visibleFraction >= 0.95 with trimmed fit", () => {
		...
		expect(result).not.toBeNull();
		const frac = computeVisibleFraction(nodes, result!.cx, result!.cy, result!.scale, 1200, 900);
