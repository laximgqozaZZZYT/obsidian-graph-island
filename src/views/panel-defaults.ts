 ...
 *
 * Each factory uses `satisfies Partial<PanelState>` so that contextual typing
 * from PanelState narrows literal types (no `as const` needed) and also
 * verifies every key/value matches the PanelState shape — preventing typos
 * and type drift without resorting to assertions.
 */
