...
import {
  type GroupPort, type Trunk, type TrunkCable, type NodePort,
  type IntraGroupCable, type CableRouteOpts, type GroupPerimInfo,
  type PolarJunctionGrid, type PortLaneInfo, type PortColorLanes,
  type CablePrepResult,
  HIGHLIGHT_CABLE_TRUNK_WIDTH, CABLE_FAN_CROWD_THRESHOLD,
  CABLE_FAN_CROWD_MIN_FRACTION, CABLE_FAN_CONNECTED_FACTOR,
  CABLE_FAN_NON_MATCH_DAMPEN, CABLE_LANE_SPACING, CABLE_LAYOUT_MARGIN,
  CABLE_OVERLAP_FRAC, TRUNK_CONDUIT_ALPHA, CABLE_CONDUIT_ALPHA,
  WIRE_BASE_ALPHA, STUB_WIRE_SPACING, MAX_CONDUIT_WIDTH,
  TRUNK_SCREEN_WIDTH, CABLE_SCREEN_WIDTH, WIRE_SCREEN_WIDTH,
  DEFAULT_CLUSTER_RADIUS, NODE_PORT_OFFSET_RATIO, NODE_PORT_MIN_OFFSET,
  zoomFadeAlpha as _zoomFadeAlpha,
  buildManhattanPath, buildHorizontalTrunkPath, buildVerticalTrunkPath,
  buildPolarTrunkPath, computeCablePath,
  computePolarCenter, computePolarJunctionGrid, filterPolarGridForPort,
  routeViaPolarGrid,
  computeGroupPorts, buildTrunks, buildIntraGroupCables,
...
function prepareCables(
  ...
  if (cache.cableDirty || !cache.cable) {
    ...
    const allGroupPorts = computeGroupPorts(groupKeys, centroids, radii, connections, cfg.coordinateSystem, polarCenter, resolvePos, cfg.nodeClusterMap ?? undefined, cache);
  ...
  if (cache.cable) {
    if (cache.intraCableDirty || !cache.intraCable) {
      ...
      if (!cache.cachedGroupPorts) {
        ...
        cache.cachedGroupPorts = computeGroupPorts(groupKeys, centroids, radii, connections, cfg.coordinateSystem, pc, resolvePos, cfg.nodeClusterMap ?? undefined, cache);
      }
    ...
    if (!cache.portColorLanes && cache.cachedGroupPorts) {
      cache.portColorLanes = buildPortColorLanes(
        cache.cable.trunks, cache.intraCable.cables, cfg, cache.cachedGroupPorts,
      );
    }
...
export function drawEdges(
  ...
  if (cfg.edgeLayerMode) {
    _drawEdgesLayered(g, filtered, resolvePos, cfg, useRelColor, isArcLayout,
      densityScale, pairCount, bundles, bundleStrength, cablePrep, arrowGfx, c);
  } else {
