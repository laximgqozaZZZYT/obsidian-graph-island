/**
 * CDP E2E Test -- Transform Expression Input
 *
 * Verifies the transform expression text input accepts expressions
 * and produces correct coordinate layouts.
 */
...
test("expression transform with cos/sin creates circular pattern", async () => {
  const result = await page.evaluate(async () => {
    ...
    panel.coordinateLayout = {
      system: "cartesian",
      axis1: { source: { kind: "index" }, transform: { kind: "expression", expr: "cos(i*2*pi/n)", scale: 200 } },
      axis2: { source: { kind: "index" }, transform: { kind: "expression", expr: "sin(i*2*pi/n)", scale: 200 } },
      perGroup: true,
    };
    ...
    if (view.pixiNodes instanceof Map) {
      for (const [, pn] of view.pixiNodes) dists.push(Math.sqrt(pn.data.x**2 + pn.data.y**2));
    }
...
test("linear transform scales positions proportionally", async () => {
  const result = await page.evaluate(async () => {
    ...
    panel.coordinateLayout = {
      system: "cartesian",
      axis1: { source: { kind: "index" }, transform: { kind: "linear", scale: 100 } },
      axis2: { source: { kind: "index" }, transform: { kind: "linear", scale: 50 } },
      perGroup: true,
    };
    if (typeof view.doRender === "function") view.doRender();
