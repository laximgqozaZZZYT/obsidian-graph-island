 ...
 */
export function pointToNearestRoad(px: number, py: number, network: RoadNetwork): number {
	...
	for (const seg of network.segments) {
		...
		if (seg.waypoints.length > 0) {
			const pts = [{ x: fromIsect.x, y: fromIsect.y }, ...seg.waypoints, { x: toIsect.x, y: toIsect.y }];
