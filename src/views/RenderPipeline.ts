...
export class RenderPipeline {
	...
	private readonly timers = new ManagedTimers();
	/** FPS tracking */
	...
	createPixiNodes(nodes: GraphNode[], nodeR: (n: GraphNode) => number, nodeColor: (n: GraphNode) => number) {
		...
		if (sorted.length > IMMEDIATE_BATCH) {
			...
			this.timers.setTimeout(() => this.host.onAllPixiNodesCreated?.(), 0);
		}
	}
	...
	private processDeferredBatch = () => {
		...
		if (this.pendingNodes.length > 0) {
			...
			this.timers.setTimeout(() => this.enrichLabelsDeferred(), 2500);
		}
	};
	...
	private enrichLabelsDeferred(): void {
		if (this._enrichmentCancelId !== null) this.timers.clear(this._enrichmentCancelId);
		...
		const processNext = () => {
			...
			if (todo.length > 0) {
				this._enrichmentCancelId = this.timers.setTimeout(processNext, 0);
			} else {
				this.cullOverlappingLabels();
				this.markDirty(true);
			}
	...
	private scheduleDeferredBatch() {
		...
		// between batches.
		this.deferredBatchId = this.timers.setTimeout(this.processDeferredBatch, 0);
	}

	cancelDeferredBatch() {
		if (this.deferredBatchId !== null) this.timers.clear(this.deferredBatchId);
		this.deferredBatchId = null;
		this.pendingNodes = [];
		this.pendingNodeR = null;
		this.pendingNodeColor = null;
