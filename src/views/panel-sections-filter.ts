  ...
  if (panel.colorEdgesByRelation && ctx.relationColors.size > 0) {
    buildSection(tabEl, t("section.relationColors"), (body) => {
      ...
      for (const [rel, color] of ctx.relationColors) {
        ...
        /* label element */ group.createEl("span", { text: rel, cls: "graph-color-group-label gi-color-group-label" });
...
// ---------------------------------------------------------------------------
function _buildCardSubSettings(
  body: HTMLElement, panel: PanelState, cb: PanelCallbacks,
): void {
...
// ---------------------------------------------------------------------------
function _buildDonutSubSettings(
  body: HTMLElement, panel: PanelState, cb: PanelCallbacks,
): void {
