...
現状を把握しました。元のissueは「stale worktrees の削除」が目的でしたが、worktreeは既に全て削除済み（メインの1つだけ）です。stash は14件残っていますが、それは別の話です。

このissueは **既に完了している** 状態です。対象のworktreeは存在せず、cleanな状態です。

分解すべき実作業がないため、以下の1タスクのみです：
