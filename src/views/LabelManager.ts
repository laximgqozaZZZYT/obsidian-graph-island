...
export class LabelManager {
	...
	private _applyDiversityAndCap(
		...
		// Desktop uses no cap; LOD + overlap culling handle density.
		...
		const visCount = this._applyVisibilityCap(candidates, rt, baseOpacity, maxVisible);
		this._enforceLabelFloor(candidates, rt, baseOpacity, zoom, visCount);
		this._applyZoomOutEmphasis(candidates, zoom);
	}

	/** Sort-then-cap pass: assign visibility based on priority + maxVisible cap.
	 *  Returns the count of visible non-super/non-hovered labels for floor logic. */
	private _applyVisibilityCap(
		candidates: { pn: PixiNode; deg: number; isSuper: boolean; isHovered: boolean }[],
		rt: RenderThresholds,
		baseOpacity: number,
		maxVisible: number,
		...
		for (const c of candidates) {
			...
			if (isHovered || isSuper) {
				...
				pn.label!.alpha = Math.max(alphaMin, baseOpacity);
	...
	/** Hide a label and its associated sub-labels (used when cap exceeded). */
	 ...
	 *  Candidates are priority-desc sorted, so re-enable hidden top entries. */
	private _enforceLabelFloor(
		candidates: { pn: PixiNode; deg: number; isSuper: boolean; isHovered: boolean }[],
		rt: RenderThresholds,
		baseOpacity: number,
		zoom: number,
		...
		if (zoom >= hardHideZoom || visCountIn >= minFloor) return;

		const alphaMin = rt.labelAlphaMin ?? 0.7;
		let visCount = visCountIn;
		for (const c of candidates) {
	...
	/** Zoom-out label emphasis: boost background opacity + padding for
	 *  surviving labels so they stand out as "important nodes" at low zoom. */
