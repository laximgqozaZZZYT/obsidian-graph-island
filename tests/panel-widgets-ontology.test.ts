...
function augmentMockEl(el: any): any {
  const augment = (node: any) => {
    ...
    if (node.style && !node.style.setProperty) {
      node.style.setProperty = (key: string, val: string) => { node.style[key] = val; };
    }
    if (!node.hasClass) {
    ...
    if (!node.insertBefore) {
      node.insertBefore = vi.fn((newNode: any, refNode: any) => { return newNode; });
    }
    if (!node.appendChild) {
      node.appendChild = vi.fn((child: any) => { node.children.push(child); return child; });
    }
    if (!node.parentNode) {
    ...
    if (origCreateEl) {
      node.createEl = (...args: any[]) => { const child = origCreateEl(...args); augment(child); return child; };
    }
    const origCreateDiv = node.createDiv?.bind(node);
    if (origCreateDiv) {
      node.createDiv = (...args: any[]) => { const child = origCreateDiv(...args); augment(child); return child; };
    }
    const origCreateSpan = node.createSpan?.bind(node);
    if (origCreateSpan) {
      node.createSpan = (...args: any[]) => { const child = origCreateSpan(...args); augment(child); return child; };
    }
  };
  augment(el);
  return el;
...
describe("renderOntologyRule", () => {
  ...
  it("disables reverse input for bidirectional relations (is-alike)", () => {
    ...
    const disabledInput = inputs.find(i =>
      i.classList?.items?.includes("is-disabled") || i.cls?.includes("is-disabled")
    );
