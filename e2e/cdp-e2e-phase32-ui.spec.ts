/**
 * Phase 32 — showAncestryBreadcrumb toggle
 * Verifies that toggling showAncestryBreadcrumb controls breadcrumb trail display.
 */
...
test.describe("Phase 32 — showAncestryBreadcrumb toggle", () => {
  test("32-1: showAncestryBreadcrumb=false is baseline", async () => {
  ...
  test("32-2: showAncestryBreadcrumb=true enables breadcrumb display", async () => {
    await page.evaluate(async () => {
      const v = (window as any).app.workspace.getLeavesOfType("graph-view").find((l: any) => "pixiNodes" in l.view)?.view;
      if (!v) return;
  ...
  test("32-3: breadcrumb requires hub nodes with paths", async () => {
    // Verify graph has high-degree hubs that would serve as ancestry roots
    const hubCount = await page.evaluate(() => {
      ...
      // Count nodes with degree >= 20 (hub threshold)
      let hubs = 0;
