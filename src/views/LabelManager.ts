...
export class LabelManager {
	...
	private _applyDiversityAndCap(
		...
		const zoom = this.host.getWorldScale();
		// Mobile lightweight mode: cap labels to 50 for performance. 0 = no cap.
		...
		this._restoreLabelFloor(candidates, rt, baseOpacity, zoom, visCount);
		this._applyZoomOutEmphasis(candidates, zoom);
	}

	/** Apply per-candidate visibility decision: hovered/super always show; others
	 *  consume one slot of `maxVisible` and are hidden once the cap is reached.
	 *  Returns the number of non-hovered, non-super labels shown. */
	private _applyVisibilityCap(
		...
		const labelAlpha = Math.max(rt.labelAlphaMin ?? 0.7, baseOpacity);
		let visCount = 0;
		for (const c of candidates) {
			const { pn, isHovered, isSuper } = c;
			if (isHovered || isSuper) {
				this._showLabel(pn, labelAlpha);
				continue;
			}
	...
	private _restoreLabelFloor(
		...
		const labelAlpha = Math.max(rt.labelAlphaMin ?? 0.7, baseOpacity);
		let restored = visCount;
		for (const c of candidates) {
			if (restored >= minFloor) break;
			const { pn, isHovered, isSuper } = c;
