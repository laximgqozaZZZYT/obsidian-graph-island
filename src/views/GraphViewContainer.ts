...
import { nextTimelineBar, type TimelineNavKey } from "../layouts/timeline-navigation";
...
export class GraphViewContainer extends ItemView implements InteractionHost, RenderHost, LayoutHost {
	...
	private _handleTimelineArrowKey(key: string): void {
		...
		const target = nextTimelineBar(bars, this.highlightedNodeId, key as TimelineNavKey);
		if (!target) return;
