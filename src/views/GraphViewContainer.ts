...
import {
	...
	lightenHex,
	initializeForceLayoutNodePositions,
...
export class GraphViewContainer extends ItemView implements InteractionHost, RenderHost, LayoutHost {
	...
	private _setupForceLayout(
		...
		// Seed positions for fade-in members (golden-angle spiral around the
		// super-node origin so the opening frame already has a pleasing radial
		// composition), reuse saved positions where valid, randomize unusable
		// coordinates, and finally pin nodes the user has anchored.
		initializeForceLayoutNodePositions(gd.nodes, {
			fade: this._fadeInTween,
		...
		const fadeTween = this._fadeInTween;
		if (fadeTween) {
			for (const id of fadeTween.stagger.keys()) {
				const pn = this.pixiNodes.get(id);
				if (!pn) continue;
				pn.gfx.scale.set(0);
				pn.gfx.alpha = 0;
