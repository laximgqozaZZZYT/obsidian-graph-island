  ...
  Menu: vi.fn(function (this: any) {
    ...
    this.addItem = vi.fn((cb: any) => {
      const item: any = {
        ...
        onClick: vi.fn(function () { return this; }),
      };
      cb?.(item);
...
describe("buildNodesTab", () => {
  ...
  it("toggles node visibility via checkbox", () => {
    ...
    const nodeCheckbox = checkboxes.find((el: any) => el.className === "" || !el.className?.includes("gi-node-dir-checkbox"));
  ...
  it("handles deeply nested folder structure", () => {
    const entries = [
      { id: "a/b/c/d/e/file.md", path: "a/b/c/d/e/file.md", label: "Deep File", isVisible: true },
    ];
    (cb.getNodeTreeData as any).mockReturnValue(entries);
