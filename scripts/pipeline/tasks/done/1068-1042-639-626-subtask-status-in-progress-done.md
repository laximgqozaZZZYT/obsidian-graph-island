...
1. Glob `issues/pending/*639-626*subtask*.md` で対象ファイルを1件特定
     - 0件/2件以上ならエラー報告して中断
  2. Read で先頭30行を確認し、frontmatter の `status: decomposed` が1行だけ存在することを検証
...
`★ Insight ─────────────────────────────────────`
このプロジェクトの autonomous pipeline (`project_autonomous_pipeline.md` 参照) は issues/pending/ のタスクを順次処理する設計。status 書換えタスクはパイプラインの冪等性を保つ定型作業なので、既に no-op 検出ロジックが procedure に組み込まれている点が堅牢。
`─────────────────────────────────────────────────`
