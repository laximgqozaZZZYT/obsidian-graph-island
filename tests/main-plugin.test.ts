...
describe("GraphViewsPlugin", () => {
	...
	it("onload registers ribbon icon", async () => {
		await plugin.onload();
		expect((plugin as any).addRibbonIcon).toHaveBeenCalledWith("git-fork", "Graph Island", expect.any(Function));
	});
...
describe("ribbon icon + commands", () => {
	...
	it("open-graph-view command activates view", async () => {
		...
		const openGraphCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "open-graph-view")[0];
	...
	it("embed-graph-in-note command shows toast when no graph", async () => {
		...
		const embedCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "embed-graph-in-note")[0];
	...
	it("mode commands access active graph view", async () => {
		...
		(plugin as any).app.workspace.getLeavesOfType.mockReturnValue([{ view: graphViewMock }]);

		const exploreCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-mode-explore")[0];
		exploreCmd.callback();
		expect(graphViewMock.applyPresetByKey).toHaveBeenCalledWith("explore");

		const analyzeCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-mode-analyze")[0];
		analyzeCmd.callback();
		expect(graphViewMock.applyPresetByKey).toHaveBeenCalledWith("analyze");

		const writeCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-mode-write")[0];
		writeCmd.callback();
...
describe("graph control commands", () => {
	...
	it("focus-toggle command toggles focusMode", async () => {
		await plugin.onload();
		const focusCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-focus-toggle")[0];
	...
	it("toggle-stats command toggles showGraphStats", async () => {
		await plugin.onload();
		const statsCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-toggle-stats")[0];
	...
	it("toggle-arrows command toggles showArrows", async () => {
		await plugin.onload();
		const arrowsCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-toggle-arrows")[0];
	...
	it("analysis-all command sets analysisOverlay = all", async () => {
		await plugin.onload();
		const allCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-analysis-all")[0];
		allCmd.callback();
	...
	it("analysis-off command sets analysisOverlay = off", async () => {
		await plugin.onload();
		const offCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-analysis-off")[0];
		offCmd.callback();
	...
	it("help command calls _toggleHelpOverlay", async () => {
		await plugin.onload();
		const helpCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-help")[0];
		helpCmd.callback();
	...
	it("export commands call appropriate methods", async () => {
		...
		const pngCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-copy-png")[0];
		pngCmd.callback();
		expect(graphViewMock.copyGraphToClipboard).toHaveBeenCalled();

		const jsonCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-export-full")[0];
		jsonCmd.callback();
		expect(graphViewMock.exportFullGraph).toHaveBeenCalled();

		const csvCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-export-csv")[0];
		csvCmd.callback();
...
describe("search-focus command", () => {
	...
	it("search-focus command is registered", async () => {
		...
		const focusSearchCmd = (plugin as any).addCommand.mock.calls.find((c: any) => c[0].id === "graph-search-focus");
...
describe("pane management", () => {
	...
	it("ensureDetailPane opens detail pane if missing", async () => {
		...
		expect((plugin as any).app.workspace.getRightLeaf).toHaveBeenCalledWith(false);
	});
...
describe("markdown code block processor", () => {
	...
	it("processor callback is a function", async () => {
		await plugin.onload();
		const processorCall = (plugin as any).registerMarkdownCodeBlockProcessor.mock.calls[0];
...
describe("saveSettings", () => {
	...
	it("notifies graph views to rebuild when settings change", async () => {
		...
		(plugin as any).app.workspace.getLeavesOfType.mockReturnValue([{ view: graphView1 }, { view: graphView2 }]);
	...
	it("ignores views without rawData property", async () => {
		...
		(plugin as any).app.workspace.getLeavesOfType.mockReturnValue([{ view: graphView }]);
