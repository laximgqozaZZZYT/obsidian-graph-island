...
import { BUNDLE_SKIP } from "../constants";
import type { CanvasGraphics } from "./canvas2d";
import type { EdgeDrawConfig, GroupBBox, BBoxFace, JunctionGrid, BundleGroup, EdgeRenderCache } from "./EdgeRenderer";
import {
	resolveEdgeColor,
	computeGraphCenter,
	computeGroupBBox,
	...
	mergeNearbyValues,
	deduplicatePath,
	buildDirectionBundles,
	drawTrunks,
	drawIntraGroupCables,
...
function collectPairData(
	...
	const pairData = new Map<string, { srcGroup: string; tgtGroup: string; byColor: Map<number, GraphEdge[]> }>();
...
function computeJunction(
	...
	return len > 1 ? { x: port.x + (dx / len) * jDist, y: port.y + (dy / len) * jDist } : { x: port.x, y: port.y };
}
...
function assembleTrunk(
	...
	const jctA = computeJunction(
		portA,
		centroids.get(pair.srcGroup),
		radii.get(pair.srcGroup) ?? DEFAULT_CLUSTER_RADIUS,
	);
	const jctB = computeJunction(
		portB,
		centroids.get(pair.tgtGroup),
		radii.get(pair.tgtGroup) ?? DEFAULT_CLUSTER_RADIUS,
	);
...
function buildGroupPerimeterMap(
	...
	for (const groupKey of allGroupKeys) {
		const info = computeGroupPerimInfo(
			groupKey,
			graphCenter,
			isPolar,
			clusterCentroids,
...
export function buildIntraGroupCables(
	...
	const groupPerimeters = buildGroupPerimeterMap(
		allGroupKeys,
		graphCenter,
		isPolar,
		clusterCentroids,
...
// ---------------------------------------------------------------------------
// Cache-aware preparation / drawing entry points
// ---------------------------------------------------------------------------
// Moved from EdgeRenderer.ts to keep god-object size in check.  Cache is now
// a required argument (no module-level fallback) — EdgeRenderer always passes
// its own _cache instance explicitly, preserving the "no module-level caches"
