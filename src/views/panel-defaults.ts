 ...
 *
 * Type strategy:
 *   Each factory uses `satisfies Partial<PanelState>` so that PanelState's
 *   field types act as contextual types — narrowing string/number literals
 *   to their declared union members and giving empty arrays/objects their
 *   element type — without per-field `as Type` casts. The final
 *   createDefaultPanelState() return is annotated `PanelState`, which is
 *   what enforces completeness across the four buckets.
 *
