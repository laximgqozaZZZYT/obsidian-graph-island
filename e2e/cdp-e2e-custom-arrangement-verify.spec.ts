/**
 * CDP E2E: Verify custom arrangement option works with coordinate engine.
 * Tests that "custom" arrangement is available and accepts axis configurations.
 */
...
test("custom arrangement accepts coordinateLayout and renders nodes", async () => {
  ...
  const result: any = await page.evaluate(ev(`
    ...
    if (!pn) return { count: 0, arrangement: "" };
    return {
      count: pn.size,
...
test("custom arrangement with polar system produces radial pattern", async () => {
  ...
  const result: any = await page.evaluate(ev(`
    const pn = view.pixiNodes;
    if (!pn || pn.size === 0) return { count: 0, radii: [] };
    const radii = [];
    let cx = 0, cy = 0;
    for (const [, n] of pn) { cx += n.data.x; cy += n.data.y; }
    cx /= pn.size; cy /= pn.size;
    for (const [, n] of pn) {
      radii.push(Math.round(Math.sqrt((n.data.x-cx)**2 + (n.data.y-cy)**2)));
    }
    radii.sort((a,b) => a-b);
    return { count: pn.size, minR: radii[0], maxR: radii[radii.length-1] };
  `));
...
test("switching from custom back to preset clears coordinateLayout effect", async () => {
  ...
  const result: any = await page.evaluate(ev(`
    return {
      arrangement: view.panel.clusterArrangement,
      coordNull: view.panel.coordinateLayout === null || view.panel.coordinateLayout === undefined,
