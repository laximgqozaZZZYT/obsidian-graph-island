...
function attachAutocomplete(input: HTMLInputElement, suggestions: string[]) {
	...
	input.addEventListener("blur", () => {
		if (blurHideTimer !== null) clearTimeout(blurHideTimer);
...
function _setupSearchJumpListeners(
	...
	let inputDeferTimer: ReturnType<typeof setTimeout> | null = null;
	let blurDismissTimer: ReturnType<typeof setTimeout> | null = null;

	input.addEventListener("input", () => {
		// Defer slightly so attachQueryHint processes first
		if (inputDeferTimer !== null) clearTimeout(inputDeferTimer);
		inputDeferTimer = setTimeout(() => {
			inputDeferTimer = null;
			ctx.rebuild();
		}, 50);
	});
