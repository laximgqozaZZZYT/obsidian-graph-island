...
import type {
  EdgeDrawConfig, GroupBBox, BBoxFace, JunctionGrid,
} from "./EdgeRenderer";
import {
  resolveEdgeColor, computeGraphCenter, computeGroupBBox,
  computePortFace, faceCenter, facePerpendicular,
  buildPerimeterPath, computeJunctionGrid,
  filterGridForPortFace, findNearestGap, findGapBetween,
  routeViaJunctionGrid, angleDist, shortestAngleDelta,
  mergeNearbyValues, deduplicatePath,
  FADE_BY_DEGREE_MIN_ALPHA, WEIGHT_THICKNESS_FACTOR,
} from "./EdgeRenderer";
...
export function buildTrunks(
  ...
  if (!ports) {
    ...
    ports = computeGroupPorts(groupKeys, centroids, _radii, connections, cfg.coordinateSystem, _polarCenter, resolvePos, cfg.nodeClusterMap ?? undefined);
  }
  ...
  for (const [pairKey, pair] of pairData) {
    ...
    const middle = isPolar && _polarCenter
 ...
 */
export function cableFadeByDegree(edges: GraphEdge[], cfg: Pick<EdgeDrawConfig, "fadeByDegree" | "maxDegree" | "degrees">): number {
