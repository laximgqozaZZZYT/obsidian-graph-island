...
/** Count occurrences of each tag across non-tag nodes (used to rank tag specificity). */
...
/** Pick the rarest tag on a node — that tag is the "most specific" for membership assignment. */
...
/** Assign each non-tag node to ONLY its most specific tag. */
...
/** Extract bidirectional `tag\0tag` pairs from inheritance/aggregation edges between tag nodes. */
