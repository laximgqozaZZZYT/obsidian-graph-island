...
export function applyConcentricLayout(graph: GraphData, options?: ConcentricLayoutOptions): ConcentricLayoutResult {
	if (graph.nodes.length === 0) {
		return { data: { nodes: [], edges: graph.edges }, shells: [] };
	}

	const { centerX = 0, centerY = 0, minRadius = 50, radiusStep = 60, sortByInDegree = false } = options ?? {};

	const nodes = graph.nodes.map((n) => ({ ...n }));
	const degrees = sortByInDegree ? computeInDegree(nodes, graph.edges) : computeNodeDegrees(nodes, graph.edges);
 ...
 */
export function repositionShell(shell: ShellInfo, nodes: Map<string, GraphNode>): void {
