...
export function extractFrontmatterImage(meta: Record<string, unknown> | undefined): string | null {
	const imgPath = meta?.image ?? meta?.thumbnail ?? meta?.cover;
	if (!imgPath || typeof imgPath !== "string") return null;
	return imgPath;
}

export function isNodeOnScreen(sx: number, sy: number, vw: number, vh: number, margin: number): boolean {
	return sx >= -margin && sx <= vw + margin && sy >= -margin && sy <= vh + margin;
}

export function createThumbnailClone(img: HTMLImageElement, sx: number, sy: number, size: number): HTMLImageElement {
...
export function resolveThumbnailUrl(path: string, vault: Vault): string | null {
