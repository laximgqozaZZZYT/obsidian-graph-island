...
作業対象は issues/ ディレクトリのみ。src/**, tests/**, 設定ファイル、God Object には触れない。
  ...
  2. Read で対象ファイル全体を確認。Edit で frontmatter の `status: done` または `status: in-progress` を `status: done` に書き換える。priority/reported/parent/depends/summary/source および本文は一切変更しない。
