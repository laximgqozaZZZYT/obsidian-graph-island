 ...
 */
export function tessellateCircle(cx: number, cy: number, r: number, segments = 24): Float32Array {
 ...
 * Returns 12 floats.
 */
export function tessellateRect(x: number, y: number, w: number, h: number): Float32Array {
 ...
 */
export function tessellateRoundedRect(x: number, y: number, w: number, h: number, r: number): Float32Array {
	// Clamp radius
 ...
 */
export function expandLineStrip(points: { x: number; y: number }[], width: number): Float32Array {
...
export function tessellateArc(
	...
	const segs = segments ?? Math.max(1, Math.ceil(absSweep / (Math.PI / 12)));
...
function pointLineDistance(px: number, py: number, lx0: number, ly0: number, lx1: number, ly1: number): number {
	const dx = lx1 - lx0;
	const dy = ly1 - ly0;
 ...
 */
export function dashifyLineStrip(points: Pt[], pattern: number[]): Pt[][] {
