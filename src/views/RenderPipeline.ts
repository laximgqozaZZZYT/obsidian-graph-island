...
export class RenderPipeline {
	...
	private _buildBatchContext(crc: ReturnType<typeof Object.assign>, rt: ReturnType<typeof Object.assign>) {
		...
		// LOD tiers (5-level + extreme/mid flags + minWorldRadius + mobile floor)
		const { isExtremeZoom, isMidZoom, minWorldRadius, lodLevel } = classifyLodTiers(
			worldScale,
			rt as Parameters<typeof classifyLodTiers>[1],
			Platform.isMobile,
		);
	...
	private _renderNodesPass(
		...
		if (isExtremeZoom) {
			renderExtremeZoomDots(g, visible, tlFilteredOut, alpha, worldScale, crc.filteredNodeAlpha);
		} else if (isMidZoom) {
			renderMidZoomCircles(g, visible, tlFilteredOut, alpha, minWorldRadius, crc.filteredNodeAlpha);
		} else {
