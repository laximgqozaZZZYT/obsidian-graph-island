...
describe("LayoutTransition", () => {
	...
	it("should set initial positions to from values on start", () => {
		...
		currentTime = 0;
		transition.start([{ data, fromX: 0, fromY: 0, toX: 100, toY: 200 }]);
	...
	it("should call onComplete callback when finished", () => {
		...
		currentTime = 0;
		transition.start([{ data, fromX: 0, fromY: 0, toX: 100, toY: 200 }], onComplete);
	...
	it("cancel does not trigger onComplete", () => {
		const onComplete = vi.fn();
		currentTime = 0;
		transition.start([{ data: { x: 0, y: 0 }, fromX: 0, fromY: 0, toX: 100, toY: 100 }], onComplete);
		transition.cancel();
