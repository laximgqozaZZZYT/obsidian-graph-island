...
// ---------------------------------------------------------------------------
function _buildBookmarkSection(tabEl: HTMLElement, panel: PanelState, ctx: PanelContext, cb: PanelCallbacks): void {
	buildSection(
		tabEl,
		t("section.bookmarks"),
		(body) => {
	...
	buildSection(
		...
		(body) => {
			...
			if (panel.nodeDisplayMode === "card") {
				...
				addSlider(
					body,
					t("display.cardMaxWidth"),
					60,
					300,
					10,
					panel.cardDisplayConfig.maxWidth ?? 120,
					(v) => {
				...
				addSlider(
					...
					(v) => {
						...
						cb.markDirty();
						cb.announceA11y?.(
							`${t("display.cardContentScale") ?? "Card Size by Content"}: ${(v * 100).toFixed(0)}%`,
						);
					},
				...
				addSlider(
					...
					1.0,
					0.05,
					(crcGE as any).plainCardFillAlpha ?? 0.8,
					(v) => {
				...
				addSlider(
					body,
					t("display.cardBodyFontSize") ?? "Body Font Size",
					4,
					16,
					1,
					rtCard.cardBodyFontSize,
					(v) => {
				...
				addSlider(
					body,
					t("display.donutInnerRadius"),
					0,
					0.9,
					0.05,
					panel.donutDisplayConfig.innerRadius ?? 0.6,
					(v) => {
...
function _buildDiscoverySection(tabEl: HTMLElement, panel: PanelState, _ctx: PanelContext, cb: PanelCallbacks): void {
	buildSection(
		tabEl,
		t("section.discovery"),
		(body) => {
...
function _buildInteractionSection(tabEl: HTMLElement, panel: PanelState, _ctx: PanelContext, cb: PanelCallbacks): void {
	buildSection(
		tabEl,
...
function _buildRoadNetworkSection(tabEl: HTMLElement, panel: PanelState, _ctx: PanelContext, cb: PanelCallbacks): void {
	buildSection(
		tabEl,
...
function _buildMinimapSection(tabEl: HTMLElement, panel: PanelState, _ctx: PanelContext, cb: PanelCallbacks): void {
	buildSection(
		tabEl,
...
function _buildGraphSyncSection(tabEl: HTMLElement, panel: PanelState, _ctx: PanelContext, cb: PanelCallbacks): void {
	buildSection(
		tabEl,
		t("section.graphSync"),
		(body) => {
...
function _buildOntologySection(tabEl: HTMLElement, _panel: PanelState, ctx: PanelContext, cb: PanelCallbacks): void {
	buildSection(
		tabEl,
		t("section.ontology"),
		(body) => {
...
function buildExprLibrary(body: HTMLElement, panel: PanelState, cb: PanelCallbacks): void {
...
function buildConstantsUI(body: HTMLElement, panel: PanelState, cb: PanelCallbacks): void {
