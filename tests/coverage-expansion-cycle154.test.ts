...
describe("applyTransform curve (cycle154)", () => {
  it("archimedean curve produces monotonically increasing values", () => {
    const raw = new Map([["a", 0], ["b", 1], ["c", 2], ["d", 3]]);
    const t = applyTransform(
      raw,
      { kind: "curve", curve: "archimedean", scale: 1 },
      10,
    );
  ...
  it("unknown curve falls back to linear spacing", () => {
    const raw = new Map([["a", 0], ["b", 5]]);
    const t = applyTransform(
      raw,
      { kind: "curve", curve: "nonexistent_curve" as any, scale: 1 },
      10,
    );
  ...
  it("curve with custom params overrides defaults", () => {
    ...
    const raw = new Map([["a", 0], ["b", 1], ["c", 2], ["d", 3], ["e", 4]]);
    const t1 = applyTransform(
      raw,
      { kind: "curve", curve: "rose", scale: 1, params: { k: 3 } },
      10,
    );
    const t2 = applyTransform(
      raw,
      { kind: "curve", curve: "rose", scale: 1, params: { k: 7 } },
      10,
    );
...
describe("applyTransform stack-avoid (cycle154)", () => {
  it("spreads nodes within same column bin", () => {
    const raw = new Map([["a", 1], ["b", 2], ["c", 3]]);
    const other = new Map([["a", 10], ["b", 10], ["c", 10]]); // same column
    const t = applyTransform(
      raw,
      { kind: "stack-avoid" },
      10,
  ...
  it("nodes in different columns get independent offsets", () => {
    const raw = new Map([["a", 1], ["b", 2], ["c", 1], ["d", 2]]);
    const other = new Map([["a", 0], ["b", 0], ["c", 100], ["d", 100]]);
...
describe("applyTransform even-divide (cycle154)", () => {
  it("distributes angles evenly per ring when otherAxisValues provided", () => {
    const raw = new Map([["a", 0], ["b", 1], ["c", 2], ["d", 3]]);
    const other = new Map([["a", 10], ["b", 10], ["c", 20], ["d", 20]]);
    const t = applyTransform(
      raw,
      { kind: "even-divide", totalRange: 360 },
      1,
      other,
    );
...
describe("applyTransform shape-fill (cycle154)", () => {
  it("square fill produces grid positions", () => {
    ...
    const t = applyTransform(
      raw,
      { kind: "shape-fill", shape: "square", axis: 1 },
      40,
    );
    // 4 nodes → 2x2 grid
    expect(t.size).toBe(4);
  ...
  it("triangle fill axis=2 returns y coordinates", () => {
    ...
    const t = applyTransform(
      raw,
      { kind: "shape-fill", shape: "triangle", axis: 2 },
      30,
    );
  ...
  it("hexagon fill places first node at center (0)", () => {
    const raw = new Map([["center", 0], ["ring1a", 1], ["ring1b", 2]]);
    const tX = applyTransform(
      raw,
      { kind: "shape-fill", shape: "hexagon", axis: 1 },
      30,
    );
    const tY = applyTransform(
      raw,
      { kind: "shape-fill", shape: "hexagon", axis: 2 },
      30,
    );
  ...
  it("diamond fill produces rotated grid positions", () => {
    ...
    const t = applyTransform(
      raw,
      { kind: "shape-fill", shape: "diamond", axis: 1 },
      40,
    );
    expect(t.size).toBe(4);
    for (const v of t.values()) {
  ...
  it("circle fill distributes nodes in concentric rings", () => {
    ...
    const t = applyTransform(
      raw,
      { kind: "shape-fill", shape: "circle", axis: 1 },
      30,
    );
...
describe("applyTransform date-to-index (cycle154)", () => {
  it("sorts by raw value and assigns sequential indices", () => {
    const raw = new Map([["c", 30], ["a", 10], ["b", 20]]);
...
describe("resolveAxisValues hop source (cycle154)", () => {
  it("assigns BFS depth from start node", () => {
    ...
    const ctx = baseCtx({
      edges,
      degrees: new Map([["start", 1], ["mid", 2], ["far", 1]]),
    });
    const vals = resolveAxisValues(
      nodes,
      { kind: "hop", from: "start", maxDepth: 5 },
      ctx,
    );
...
describe("resolveAxisValues property source (cycle154)", () => {
  ...
  it("falls back to meta field when getNodeProperty is absent", () => {
    const nodes = [
      makeNode("a", { priority: "5" }),
      makeNode("b", { priority: "15" }),
    ];
    const ctx = baseCtx();
    const vals = resolveAxisValues(
      nodes,
...
describe("toCartesian polar multi-node (cycle154)", () => {
  it("polar with θ=0 puts nodes along x-axis", () => {
    const r = new Map([["a", 100], ["b", 200]]);
    const theta = new Map([["a", 0], ["b", 0]]);
...
describe("computeAutoFitSpacing (cycle154)", () => {
  ...
  it("constrained mode (skipGroupOverlap) limits max spacing", () => {
    const nodes = Array.from({ length: 5 }, (_, i) => ({
      id: `n${i}`, label: `N${i}`, x: 0, y: 0, vx: 0, vy: 0,
      tags: ["t"], category: "A",
    } as GraphNode));
    const cfg = baseCfg({
...
describe("applyTransform expression (cycle154)", () => {
  it("invalid expression falls back to linear", () => {
    ...
    const t = applyTransform(
      raw,
      { kind: "expression", expr: ")))invalid(((", scale: 1 },
      10,
    );
    // Fallback: v * spacing
    expect(t.get("a")).toBe(10);
  ...
  it("expression with built-in pi constant", () => {
    ...
    const t = applyTransform(
      raw,
      { kind: "expression", expr: "pi", scale: 1 },
      1,
    );
