...
export function routeViaJunctionGrid(
	...
	const _rowGap = findGapBetween(grid.rowGaps, from.y, to.y);
 ...
 */
function buildManhattanPath(a: { x: number; y: number }, b: { x: number; y: number }): { x: number; y: number }[] {
	const dx = b.x - a.x;
	const dy = b.y - a.y;
...
function buildVerticalTrunkPath(a: { x: number; y: number }, b: { x: number; y: number }): { x: number; y: number }[] {
	const dx = b.x - a.x,
		dy = b.y - a.y;
	const dist = Math.sqrt(dx * dx + dy * dy);
	if (dist < 1) return [a, b];
...
function buildIntraGroupCables(
	...
	for (const groupKey of allGroupKeys) {
		...
		if (!bbox) {
			...
			cache.groupBBox.set(groupKey, bbox);
		}
		if (!bbox) continue;
...
function getPortLaneEndpoint(info: PortLaneInfo, color: number, laneSpacing: number): { x: number; y: number } | null {
...
function _drawEdgesSinglePass(
	...
	for (const e of edges) {
		...
		const _edgeStyle = resolveEdgeStyle(e, src, tgt, cfg, densityScale, pairCount);
		let alpha = _edgeStyle.alpha;
		let lineThick = _edgeStyle.lineThick;
		const edgeHL = _edgeStyle.isHighlighted;
...
function _drawEdgesLayered(
	...
	for (let li = 0; li < EDGE_LAYER_ORDER.length; li++) {
		...
		for (const e of edges) {
			...
			let lineColor = resolveEdgeColor(e, useRelColor, cfg.relationColors, cfg.isDark);
			const _edgeStyle2 = resolveEdgeStyle(e, src, tgt, cfg, densityScale, pairCount);
			let alpha = _edgeStyle2.alpha;
			let lineThick = _edgeStyle2.lineThick;
			const edgeHLL = _edgeStyle2.isHighlighted;
