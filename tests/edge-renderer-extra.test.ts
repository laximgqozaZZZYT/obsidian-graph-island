...
describe("resolveEdgeColor (extra)", () => {
	...
	it("uses type fallback when useRelColor but no relation match", () => {
		const color = resolveEdgeColor({ source: "a", target: "b", type: "link" } as any, true, new Map(), true);
	...
	it("uses default color for light theme", () => {
		const color = resolveEdgeColor({ source: "a", target: "b" } as any, false, new Map(), false);
...
describe("shouldSkipEdge (extra)", () => {
	...
	it("skips link edge when showLinks=false", () => {
		expect(shouldSkipEdge({ source: "a", target: "b", type: "link" } as any, makeCfg({ showLinks: false }))).toBe(
			true,
		);
	});

	it("keeps link edge when showLinks=true", () => {
		expect(shouldSkipEdge({ source: "a", target: "b", type: "link" } as any, makeCfg({ showLinks: true }))).toBe(
			false,
		);
	});
	...
	it("skips similar when showSimilar=false", () => {
		expect(
			shouldSkipEdge({ source: "a", target: "b", type: "similar" } as any, makeCfg({ showSimilar: false })),
		).toBe(true);
	});
	...
	it("skips named-relation when showNamedRelation=false", () => {
		expect(
			shouldSkipEdge(
				{ source: "a", target: "b", type: "named-relation" } as any,
				makeCfg({ showNamedRelation: false }),
			),
		).toBe(true);
	});
	...
	it("keeps named-relation when showNamedRelation=true", () => {
		expect(
			shouldSkipEdge(
				{ source: "a", target: "b", type: "named-relation" } as any,
				makeCfg({ showNamedRelation: true }),
			),
		).toBe(false);
	});
...
describe("buildBidirectionalSet (extra)", () => {
	...
	it("single self-loop is not bidir (only one edge)", () => {
		const edges = [{ source: "a", target: "a", type: "link" }] as any[];
		const bidir = buildBidirectionalSet(edges);
...
describe("shouldSkipByDirection (extra)", () => {
	it("returns false for 'all' filter (shows everything)", () => {
		const result = shouldSkipByDirection({ source: "a", target: "b" } as any, {
			edgeDirectionFilter: "all",
			_bidirectionalSet: new Set(),
		});
	...
	it("returns true for 'bidirectional' filter when edge is not bidir", () => {
		const result = shouldSkipByDirection({ source: "a", target: "b" } as any, {
			edgeDirectionFilter: "bidirectional",
			_bidirectionalSet: new Set(),
		});
	...
	it("returns true for 'unidirectional' filter when edge is bidir", () => {
		const result = shouldSkipByDirection({ source: "a", target: "b" } as any, {
			edgeDirectionFilter: "unidirectional",
			_bidirectionalSet: new Set(["a→b"]),
		});
		expect(result).toBe(true);
	});
