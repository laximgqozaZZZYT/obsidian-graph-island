...
import {
	computeSunburstArcs,
	applySunburstLayout,
	buildSunburstFromGraphNodes,
	getGroupingPath,
	assignValues,
	maxDepth,
	collectFilePaths,
	countDirectChildren,
} from "../src/layouts/sunburst";
...
function sampleRoot(): SunburstData {
	return {
		...
		children: [
			...
			{
				name: "Locations",
				children: [{ name: "Castle", value: 1, filePath: "castle.md" }],
			},
		],
	};
}
...
describe("applySunburstLayout", () => {
	it("positions nodes at arc centroids", () => {
		...
		const result = applySunburstLayout({ nodes, edges }, root, { width: 800, height: 600, groupField: "category" });
	...
	it("places unmatched nodes on outer ring", () => {
		...
		const result = applySunburstLayout({ nodes, edges: [] }, root, {
			width: 800,
			height: 600,
			groupField: "category",
		});
	...
	it("preserves edges", () => {
		const nodes = [makeNode("alice", { filePath: "alice.md" }), makeNode("bob", { filePath: "bob.md" })];
...
describe("buildSunburstFromGraphNodes", () => {
	...
	it("collapses single-child chains", () => {
		...
		const nodes = [makeNode("x", { filePath: "a/b/c/file.md" })];
		const result = buildSunburstFromGraphNodes(nodes);
	...
	it("creates leaf nodes with filePath", () => {
		const nodes = [makeNode("hero", { filePath: "chars/hero.md" })];
		const result = buildSunburstFromGraphNodes(nodes);
		// Navigate to leaf
	...
	it("handles nodes without filePath (first-letter fallback)", () => {
		const nodes = [makeNode("alpha"), makeNode("bravo")];
		const result = buildSunburstFromGraphNodes(nodes);
		const topNames = result.children!.map((c) => c.name);
...
describe("assignValues", () => {
	...
	it("recursively assigns for deep hierarchy", () => {
		const root: SunburstData = {
			name: "root",
			children: [{ name: "group", children: [{ name: "x" }, { name: "y" }, { name: "z" }] }],
		};
...
describe("maxDepth", () => {
	...
	it("returns max of unbalanced branches", () => {
		const tree: SunburstData = {
			name: "r",
			children: [{ name: "shallow" }, { name: "deep", children: [{ name: "d1", children: [{ name: "d2" }] }] }],
		};
...
describe("applySunburstLayout edge cases", () => {
	...
	it("falls back when root has only 1 direct child (Uncategorized)", () => {
		const nodes = [makeNode("a", { filePath: "a.md" }), makeNode("b", { filePath: "b.md" })];
		...
		const result = applySunburstLayout({ nodes, edges: [] }, singleChildRoot, {
			width: 400,
			height: 400,
			groupField: "category",
		});
	...
	it("multiple unmatched nodes are spread evenly around outer ring", () => {
		const nodes = [makeNode("orphan1"), makeNode("orphan2"), makeNode("orphan3"), makeNode("orphan4")];
		const root = sampleRoot();
		const result = applySunburstLayout({ nodes, edges: [] }, root, {
			width: 800,
			height: 800,
		...
		const dists = result.data.nodes.map((n) => Math.sqrt((n.x - result.cx) ** 2 + (n.y - result.cy) ** 2));
...
describe("buildSunburstFromGraphNodes edge cases", () => {
	...
	it("deeply nested file paths produce collapsed chain names with slashes", () => {
		const nodes = [makeNode("x", { filePath: "a/b/c/d/e/file.md" })];
		const result = buildSunburstFromGraphNodes(nodes);
