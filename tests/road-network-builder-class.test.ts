...
describe("RoadNetworkBuilder topology dispatch", () => {
	...
	it("uses phantom builder when simulation has phantom nodes", () => {
		...
		const host = createMockHost(nodes, {
			groupGuides: [{ guide: { type: "concentric", rings: [10] }, centerX: 0, centerY: 0 }],
		});
	...
	it("dispatches to coordinate topology (polar) when isPolarArrangement=true", () => {
		const nodes = [mkNode("A", 10, 0), mkNode("B", 0, 20), mkNode("C", -30, 0), mkNode("D", 0, -40)];
		const guides = [
			{
				guide: {
					type: "coordinate",
