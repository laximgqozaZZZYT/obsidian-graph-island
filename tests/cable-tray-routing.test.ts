...
function makeChainNetwork(): RoadNetwork {
	...
	adjacency.set(1, [
		{ to: 0, weight: 5, segIdx: 0 },
		{ to: 2, weight: 3, segIdx: 1 },
	]);
...
describe("routeEdge", () => {
	...
	it("should route between two nearby nodes on grid", () => {
		const nodes = [mockNode("a", 0, 0), mockNode("b", 10, 0)];
	...
	it("should route across multiple segments", () => {
		const nodes = [mockNode("start", 0, 0), mockNode("end", 20, 20)];
	...
	it("should route on polar network", () => {
		const nodes = [mockNode("inner", 0, 0), mockNode("outer", 30, 0)];
