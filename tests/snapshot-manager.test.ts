...
describe("saveSnapshot", () => {
	...
	it("shows limit toast when 10 snapshots already exist", () => {
		const host = createMockHost();
		host.plugin.settings.snapshots = Array.from({ length: 10 }, (_, i) => createMockSnapshot(`S${i}`));
		saveSnapshot(host);
...
describe("deleteSnapshot", () => {
	...
	it("removes snapshot at valid index", () => {
		const host = createMockHost();
		host.plugin.settings.snapshots = [createMockSnapshot("S0"), createMockSnapshot("S1"), createMockSnapshot("S2")];
