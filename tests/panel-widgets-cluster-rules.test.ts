...
beforeAll(() => {
  ...
  (globalThis.document as any).createElement = (tag: string) => {
    ...
    Object.assign(el, {
      ...
      appendChild: vi.fn((n: any) => { el.children.push(n); return n; }),
      closest: () => el,
...
function augmentEl(el: any): any {
  const augment = (node: any) => {
    if (!node) return;
    if (node.style && !node.style.setProperty) node.style.setProperty = (k: string, v: string) => { node.style[k] = v; };
    if (!node.hasClass) node.hasClass = (c: string) => (node.cls ?? "").includes(c);
    if (!node.toggleClass) node.toggleClass = (c: string, force: boolean) => {
      if (!node.classList?.items) return;
      if (force) node.classList.items.push(c);
      else { const idx = node.classList.items.indexOf(c); if (idx >= 0) node.classList.items.splice(idx, 1); }
    };
    if (!node.closest) node.closest = () => null;
    if (!node.parentElement) node.parentElement = node;
    if (!node.parentNode) node.parentNode = node;
    if (!node.getBoundingClientRect) node.getBoundingClientRect = () => ({ left: 0, top: 0, right: 0, bottom: 0, width: 0, height: 0 });
    if (!node.insertBefore) node.insertBefore = vi.fn((n: any) => n);
    if (!node.appendChild) node.appendChild = vi.fn((c: any) => { node.children.push(c); return c; });
    ...
    if (origCreateEl) node.createEl = (...a: any[]) => { const c = origCreateEl(...a); augment(c); return c; };
    const origCreateDiv = node.createDiv?.bind(node);
    if (origCreateDiv) node.createDiv = (...a: any[]) => { const c = origCreateDiv(...a); augment(c); return c; };
    const origCreateSpan = node.createSpan?.bind(node);
    if (origCreateSpan) node.createSpan = (...a: any[]) => { const c = origCreateSpan(...a); augment(c); return c; };
  };
  augment(el);
  return el;
}

function makeContainer(): any { return augmentEl(createMockEl()); }
