...
親 issue の指示に従い、対象 issue ファイルを pending → done へ移動する原子的操作を実施する。
  ...
  3. 移動後のファイルの frontmatter `status: in-progress` または `status: pending` を `status: done` に書き換え（1行のみ編集）
  4. `git add` + 単一コミット（メッセージ例: `chore: mark <file> as done`）
  5. この subtask 自身のファイルも frontmatter `status: in-progress` → `status: done` に更新し、同コミットまたは追随コミットで done/ へ移動
