...
export function giDiag<T extends { nodes: { length: number }; edges: { length: number } }>(stage: string, data: T): T {
	...
	if (env || w?.__GI_DIAG__ === true) {
		// eslint-disable-next-line no-console -- gated diagnostic: only fires when NODE_ENV!=='production' or window.__GI_DIAG__===true; esbuild drops console in prod build
