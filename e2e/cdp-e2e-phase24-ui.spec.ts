/**
 * Phase 24 — fadeEdgesByDegree toggle
 * Verifies that toggling fadeEdgesByDegree changes edge alpha distribution.
 */
...
test.beforeAll(async ({}, testInfo) => {
  ...
  await page.evaluate(async () => {
    ...
    v.panel.fadeEdgesByDegree = false;
    v.rawData = null;
    v.doRender();
  });
...
test.describe("Phase 24 — fadeEdgesByDegree toggle", () => {
  test("24-1: fadeEdgesByDegree=false is baseline", async () => {
  ...
  test("24-2: fadeEdgesByDegree=true enables degree-based alpha", async () => {
    await page.evaluate(async () => {
      const v = (window as any).app.workspace.getLeavesOfType("graph-view")[0]?.view;
      if (!v) return;
      v.panel.fadeEdgesByDegree = true;
      v.rawData = null;
      v.doRender();
    });
    await page.waitForTimeout(6000);
