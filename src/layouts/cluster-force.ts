...
export type ArrangementGuide = TimelineGuide | GridGuide | TriangleGuide | ConcentricGuide | CoordinateGuide;
...
export function buildClusterForce(
	...
	const NEEDS_LAYOUT: Set<string> = new Set([
		ARRANGEMENT_CONCENTRIC,
		ARRANGEMENT_RADIAL,
		ARRANGEMENT_RANDOM,
		ARRANGEMENT_TIMELINE,
	...
	if (cfg.groupRules.length === 0 && !isGlobalLayout && !NEEDS_LAYOUT.has(cfg.arrangement) && !cfg.orphanClusterField)
		return null;
	...
	const { targets, allBars, allSequenceEdges, ringConstraints, timelineRoutes, groupGuides } = computeAbsoluteTargets(
		groups,
		edges,
		degrees,
		cfg,
	...
	return {
		...
		metadata: {
			...
			clusterRadii,
			timelineBars,
			sequenceEdges: allSequenceEdges,
			timelineRoutes,
			groupGuides,
		},
	};
...
function mergeSmallGroups(groups: Map<string, GraphNode[]>, nodeCount: number): Map<string, GraphNode[]> {
	...
	for (const [key, members] of groups) {
		...
		for (const n of members) {
			if ((degrees.get(n.id) || 0) === 0) {
				...
				if (!arr) {
					arr = [];
					orphanBuckets.set(bucketKey, arr);
				}
				arr.push(n);
			} else {
				nonOrphans.push(n);
	...
	if (lsf > 0 && nodes.length > 0) {
		...
		const extents = nodes.map((n) =>
			estimateLabelExtent(
				...
				maxDeg,
				lsf,
				cfg.nodeLabelFontSizeMin ?? 11,
				cfg.nodeLabelFontSizeMax ?? 14,
			),
		);
...
function resolveGapsAndOverlaps(
	...
	if (minGap > 0 || lsfIntra > 0) {
		resolveIntraGroupGaps(
			...
			cfg.nodeSize,
			degrees,
			cfg.maxNodeRadius ?? 60,
			cfg.minNodeRadius ?? 18,
			lsfIntra,
			cfg.nodeLabelFontSizeMin ?? 11,
			cfg.nodeLabelFontSizeMax ?? 14,
		);
...
function buildClusterMetadataFromTargets(
	...
	for (const [key, members] of groups) {
		...
		const estimated = estimateGroupRadius(
			members.length,
			cfg.nodeSize,
			cfg.nodeSpacing,
			cfg.groupScale,
...
function resolveGroupOverlapsIfNeeded(
	...
	const glHullOff = 24;
	const groupLabelPad = lsf > 0 ? ((cfg.nodeLabelFontSizeMax ?? 14) * glScaleMax + glHullOff) * lsf : 0;
...
function buildClusterForceFunction(
	...
	return (_alpha: number) => {
		const blend = isStructured ? blendInitial : blendInitial * Math.min(1, _alpha * decayFactor);
...
export function estimateLabelExtent(
	...
	const padX = isSuperNode ? LABEL_PAD.SUPER_X : LABEL_PAD.REGULAR_X;
...
export function effectiveRadius(
	...
	if (n.collapsedMembers && n.collapsedMembers.length > 0) {
		return Math.max(
			Math.min(Math.max(baseR, baseR * (1 + Math.sqrt(n.collapsedMembers.length) * 0.5)), cap),
			minNodeRadius,
		);
	}
...
function computeAbsoluteTargets(
	...
	if (cfg.coordinateLayout && !cfg.coordinateLayout.perGroup) {
		...
		return {
			targets,
			allBars: [],
			allSequenceEdges: undefined,
			groupGuides: globalGuides.length > 0 ? globalGuides : undefined,
		};
	}
	...
	if (hasHierarchy) {
		...
		return {
			targets: r.targets,
			allBars: r.allBars,
			allSequenceEdges: r.allSequenceEdges,
			timelineRoutes: r.timelineRoutes,
			groupGuides: r.groupGuides,
		};
	}
	...
	return {
		...
		allBars: r.allBars,
		allSequenceEdges: r.allSequenceEdges,
		ringConstraints: r.ringConstraints,
		timelineRoutes: r.timelineRoutes,
		groupGuides: r.groupGuides,
	};
}
...
function computeFlatTargets(
	...
	const { groupResults, actualRadii } = _computeGroupOffsetsAndRadii(groupKeys, groups, degrees, edges, cfg);
...
function computeUnifiedTimelineTargets(
	...
	const cmp =
		cfg.sortComparator ?? ((a: GraphNode, b: GraphNode) => (degrees.get(b.id) || 0) - (degrees.get(a.id) || 0));
	...
	const unified = timelineOffsetsV2({
		...
		maxGroupNodeR,
		cmp,
		nodeSpacingMap: cfg.nodeSpacingMap,
		cfg,
	});
...
function computeHierarchicalTargets(
	...
	const { groupResults: allGroupResults, actualRadii: allGroupRadii } = _computeGroupOffsetsAndRadii(
		[...groups.keys()],
		groups,
		degrees,
		edges,
...
function _sortChildKeys(childKeys: string[], groups: Map<string, GraphNode[]>, cfg: ClusterForceConfig): string[] {
	...
	return [...childKeys].sort((a, b) => (groups.get(b)?.length ?? 0) - (groups.get(a)?.length ?? 0));
}
...
function applyGroupRule(
	...
	for (const [parentKey, members] of groups) {
		...
		const finalSubs = rule.recursive ? splitByConnectedComponents(subGroups, edges) : subGroups;
 ...
 */
function splitByConnectedComponents(groups: Map<string, GraphNode[]>, edges: GraphEdge[]): Map<string, GraphNode[]> {
	const result = new Map<string, GraphNode[]>();
...
function computeOffsets(
	...
	if (HARDCODED_ARRANGEMENTS.has(cfg.arrangement)) {
		...
		const isDefault = !cfg.coordinateLayout || JSON.stringify(cfg.coordinateLayout) === JSON.stringify(preset);
		if (isDefault) {
			result = dispatchHardcoded(
				cfg.arrangement,
				members,
				degrees,
				edges,
...
function normalizeSpread(
	...
	const SKIP_NORMALIZE: Set<string> = new Set([
		ARRANGEMENT_RANDOM,
		ARRANGEMENT_TIMELINE,
		ARRANGEMENT_PHYLLOTAXIS,
		ARRANGEMENT_RADIAL,
...
function dispatchHardcoded(
	...
	switch (arrangement) {
		case ARRANGEMENT_CONCENTRIC:
			return concentricOffsets(p);
		case ARRANGEMENT_RADIAL:
			return radialOffsets(p);
		case ARRANGEMENT_GRID:
			return gridOffsets(p);
		case ARRANGEMENT_TRIANGLE:
			return triangleOffsets(p);
		case ARRANGEMENT_RANDOM:
			return { offsets: randomOffsets(p) };
		case ARRANGEMENT_TIMELINE:
			return timelineOffsetsV2(p);
		case ARRANGEMENT_EGO:
			return egoOffsets(p);
		default:
...
export function computeAutoOptimize(
	...
	if (overlapRatio <= cfg.overlapThreshold) {
		return {
			...
			repelForce: currentRepel,
			linkDistance: currentLinkDist,
			needsMore: false,
		};
	}
