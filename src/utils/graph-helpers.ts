...
/** Count how often each tag appears across non-tag nodes. */
export function countTagOccurrences(nodes: readonly GraphNode[]): Map<string, number> {
	const counts = new Map<string, number>();
	for (const n of nodes) {
		if (n.isTag || !n.tags) continue;
		for (const tag of n.tags) {
			incCounter(counts, tag);
		}
	}
	return counts;
}

/** Pick the most specific (smallest-count) tag from a node's tag list. */
export function pickMostSpecificTag(tags: readonly string[], tagCounts: Map<string, number>): string {
	let bestTag = tags[0];
...
/** Build tag↔tag relationship pairs from inheritance/aggregation edges between tag nodes. */
export function buildTagRelPairs(edges: readonly GraphEdge[]): Set<string> {
