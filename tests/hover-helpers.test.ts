...
describe("groupOffScreenNeighbors", () => {
	it("groups off-screen neighbors by cluster key", () => {
		...
		const result = groupOffScreenNeighbors(neighbors, getNode, 1, 0, 0, 800, 600, 10, getClusterKey);
	...
	it("extracts folder from filePath for default cluster key", () => {
		...
		groupOffScreenNeighbors(neighbors, getNode, 1, 0, 0, 800, 600, 10, getClusterKey);
...
describe("computeTooltipEdgePosition", () => {
	...
	it("enforces minimum distance of 40", () => {
		...
		const dist = Math.sqrt(Math.pow(pos.tipX - 400, 2) + Math.pow(pos.tipY - 300, 2));
...
describe("findSharedTagNodes", () => {
	...
	it("handles nodes with undefined tags", () => {
		const hoveredTags = ["tag1"];
		const nodes: HoverHighlightNode[] = [{ id: "n1", tags: ["tag1"] }, { id: "n2" }];
		const result = findSharedTagNodes(hoveredTags, "hovered", nodes);
		expect(result).toContain("n1");
		expect(result).not.toContain("n2");
	});
...
describe("findSameFolderNodes", () => {
	...
	it("includes hovered node when it matches folder", () => {
		...
		const nodes: HoverHighlightNode[] = [{ id: "hovered", filePath: "folder1/other.md" }];
		const result = findSameFolderNodes(hoveredPath, "hovered", nodes);
	...
	it("returns empty when hovered path has no folder", () => {
		...
		const nodes: HoverHighlightNode[] = [{ id: "n1", filePath: "any/file.md" }];
		const result = findSameFolderNodes(hoveredPath, "hovered", nodes);
		expect(result).toEqual([]);
	});
	...
	it("handles nodes with undefined filePath", () => {
		const hoveredPath = "folder1/file.md";
		const nodes: HoverHighlightNode[] = [{ id: "n1", filePath: "folder1/other.md" }, { id: "n2" }];
		const result = findSameFolderNodes(hoveredPath, "hovered", nodes);
		expect(result).toContain("n1");
		expect(result).not.toContain("n2");
	});
...
describe("computeCardBBox", () => {
	...
	it("scales dimensions by viewport scale", () => {
		...
		expect(Math.abs(bbox1.maxX - bbox1.minX)).toBeGreaterThan(Math.abs(bbox2.maxX - bbox2.minX));
	});
});
...
describe("computeTimelineFit", () => {
	...
	it("scales down when content exceeds viewport", () => {
		const bars = [{ xStart: 0, xEnd: 10000, yCenter: 5000, barHeight: 100 }];
		const fit = computeTimelineFit(bars, 1000, 600);
		expect(fit!.scale).toBeLessThan(1);
	});

	it("caps scale at 2 even for small content", () => {
		const bars = [{ xStart: 0, xEnd: 1, yCenter: 0, barHeight: 1 }];
		const fit = computeTimelineFit(bars, 1000, 600);
		expect(fit!.scale).toBeLessThanOrEqual(2);
	});

	it("centers on content", () => {
		const bars = [{ xStart: 100, xEnd: 200, yCenter: 150, barHeight: 50 }];
		const fit = computeTimelineFit(bars, 1000, 600);
	...
	it("applies 10% margin", () => {
		const bars = [{ xStart: 0, xEnd: 100, yCenter: 50, barHeight: 100 }];
		const fit = computeTimelineFit(bars, 1000, 600);
...
describe("groupOffScreenNeighbors - additional edge cases", () => {
	...
	it("applies world scale correctly", () => {
		...
		const getClusterKey = () => "cluster";
		const result1 = groupOffScreenNeighbors(neighbors, getNode, 1, 0, 0, 800, 600, 10, getClusterKey);
		const result2 = groupOffScreenNeighbors(neighbors, getNode, 2, 0, 0, 800, 600, 10, getClusterKey);
		expect(result2.get("cluster")?.avgSx).toBe(2 * (result1.get("cluster")?.avgSx || 0));
	});
});
...
describe("findSharedTagNodes - additional edge cases", () => {
	...
	it("handles empty hovered tags", () => {
		...
		const nodes: HoverHighlightNode[] = [{ id: "n1", tags: ["tag1"] }];
		const result = findSharedTagNodes(hoveredTags, "hovered", nodes);
		expect(result).toEqual([]);
	});
	...
	it("finds duplicates when multiple tags match", () => {
		const hoveredTags = ["tag1", "tag2"];
		const nodes: HoverHighlightNode[] = [{ id: "n1", tags: ["tag1", "tag2"] }];
		const result = findSharedTagNodes(hoveredTags, "hovered", nodes);
		expect(result).toContain("n1");
	});
});
...
describe("computeCardBBox - additional edge cases", () => {
	...
	it("accounts for number of fields in height", () => {
		const nodes = [{ x: 0, y: 0, radius: 5 }];
		const bbox1 = computeCardBBox(nodes, 1, 1, { tableHeaderHeight: 30, fieldLineHeight: 20, cardPadding: 10 }, 2);
		const bbox2 = computeCardBBox(nodes, 1, 1, { tableHeaderHeight: 30, fieldLineHeight: 20, cardPadding: 10 }, 5);
		expect(Math.abs(bbox2.maxY - bbox2.minY)).toBeGreaterThan(Math.abs(bbox1.maxY - bbox1.minY));
	});
});
...
describe("findSameFolderNodes - additional edge cases", () => {
	it("normalizes folder path with trailing slashes", () => {
		const hoveredPath = "folder1/file.md";
		const nodes: HoverHighlightNode[] = [{ id: "n1", filePath: "folder1/" }];
		const result = findSameFolderNodes(hoveredPath, "hovered", nodes);
		expect(result).toContain("n1");
	});
