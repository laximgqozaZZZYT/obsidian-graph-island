/**
 * CDP E2E Test -- Expression Shapes
 *
 * Verifies that grid and triangle fills expressed via math expressions
 * produce correct geometric patterns.
 */
...
test("grid fill creates rows and columns", async () => {
  const result = await page.evaluate(async () => {
    ...
    if (view.pixiNodes instanceof Map) {
      for (const [, pn] of view.pixiNodes) {
        xs.add(Math.round(pn.data.x / 30));
        ys.add(Math.round(pn.data.y / 30));
      }
    }
    return { distinctCols: xs.size, distinctRows: ys.size };
  });
