...
/** Pick the smallest-count (most specific) tag from a node's tag list. */
...
/** Assign each non-tag node to its single most specific tag. */
function assignNodesToTags(nodes: readonly GraphNode[], tagCounts: Map<string, number>): Map<string, Set<string>> {
	const membership = new Map<string, Set<string>>();
	for (const n of nodes) {
		if (n.isTag || !n.tags || n.tags.length === 0) continue;
		addToMapSet(membership, pickMostSpecificTag(n.tags, tagCounts), n.id);
	}
	return membership;
}

/** Collect symmetric (a↔b) tag relationship pairs from inheritance/aggregation edges. */
function buildTagRelPairs(edges: readonly GraphEdge[]): Set<string> {
...
export function buildTagMembership(
	...
	const tagMembership = assignNodesToTags(nodes, tagCounts);
	const tagRelPairs = buildTagRelPairs(edges);
