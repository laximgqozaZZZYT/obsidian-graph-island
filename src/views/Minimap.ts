...
// Minimap drawing constants moved to ../constants.
...
export class Minimap {
	...
	draw() {
		...
		this.wrapper.setAttribute(
			"aria-label",
			`Graph minimap — ${nodes.length} nodes — drag to navigate, arrow keys to pan`,
		);
		if (nodes.length === 0) {
	...
	private panToMinimapClick(e: MouseEvent) {
		...
		const wt = this.host.getWorldTransform();
		const vp = this.host.getViewportSize();
		this.host.setWorldPosition(vp.width / 2 - worldX * wt.scaleX, vp.height / 2 - worldY * wt.scaleY);
		this.host.wakeRenderLoop();
	}
