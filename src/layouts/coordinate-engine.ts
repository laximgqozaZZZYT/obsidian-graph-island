 ...
 */
function assignNumericWithMissingAtEnd(rawValues: { id: string; raw: string }[], result: Map<string, number>) {
...
function resolveSourceField(members: GraphNode[], field: string, result: Map<string, number>): void {
...
function buildAdjacencyList(memberSet: Set<string>, edges: GraphEdge[]): Map<string, string[]> {
...
function bfsFromHighestDegree(members: GraphNode[], ctx: CoordinateContext): Map<string, number> {
	const memberSet = new Set(members.map((m) => m.id));
	const adj = buildAdjacencyList(memberSet, ctx.edges);
...
export function resolveAxisValues(
	...
	switch (source.kind) {
		...
		case SOURCE_HOP: {
			resolveSourceHop(members, source.from.toLowerCase(), source.maxDepth ?? Infinity, ctx, result);
			break;
		}
...
function resolveCoordinateGrid(
	...
	const defaultShape1: GridShape = layout.system === "polar" ? { kind: "circle" } : { kind: "line" };
	const defaultShape2: GridShape = layout.system === "polar" ? { kind: "radial" } : { kind: "line" };
...
export function coordinateOffsets(
	...
	const axis2NeedsOther =
		layout.axis2.transform.kind === TRANSFORM_STACK_AVOID || layout.axis2.transform.kind === TRANSFORM_EVEN_DIVIDE;
	...
	const finalT1 =
		layout.axis1.transform.kind === TRANSFORM_STACK_AVOID
	...
	if (offsets.size > 0) {
		resolveCoordinateGrid(layout, members, ctx, finalT1, t2, spacing, offsets, guide);
	}
...
function computeShapeFill(shape: ShapeFillKind, n: number, sp: number): Point2D[] {
	switch (shape) {
		case SHAPE_FILL_SQUARE:
			return squareFill(n, sp);
		case SHAPE_FILL_TRIANGLE:
			return triangleFill(n, sp);
		case SHAPE_FILL_HEXAGON:
			return hexagonFill(n, sp);
		case SHAPE_FILL_DIAMOND:
			return diamondFill(n, sp);
		case SHAPE_FILL_CIRCLE:
			return circleFill(n, sp);
	}
}
