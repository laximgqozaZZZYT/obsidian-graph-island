...
if [[ -z "${SKIP_DIRTY_CHECK:-}" && -n "$(git status --porcelain)" ]]; then
  ...
  echo "SKIP: working tree dirty — autonomous probably mid-cycle (shared count: $(dirty_skip_bump))"
  exit 0
fi
...
if [[ $APPLY -eq 1 && $? -eq 0 ]]; then
  # Phase R4 (2026-05-02): wrap the csv_insert + commit pair under the
  # csv-commit flock so a parallel autonomous cycle can never observe
  # the dirty state in between. Same lock file as csv_atomic_* helpers.
  (
    flock 9
    if [[ -n "$(git status --porcelain scripts/pipeline/issues.csv scripts/pipeline/descriptions/)" ]]; then
      ...
      git commit --no-verify -m "chore(proposer): file feature-proposal issues + archive rejects
...
with the critic's score and reason for future tuning." || true
  ...
  ) 9>>/tmp/graph-island-csv-commit.lock
fi
