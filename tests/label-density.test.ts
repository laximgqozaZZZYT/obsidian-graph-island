...
describe("computeDensityScale (label density)", () => {
	...
	it("is monotonically decreasing as zoom increases", () => {
		...
		for (let i = 1; i < zooms.length; i++) {
			expect(computeDensityScale(zooms[i], T)).toBeLessThanOrEqual(computeDensityScale(zooms[i - 1], T));
		}
	});
...
describe("generateDisplacementOffsets", () => {
	...
	it("offsets scale with node radius", () => {
		...
		expect(Math.abs(large[0].dx) + Math.abs(large[0].dy)).toBeGreaterThan(
			Math.abs(small[0].dx) + Math.abs(small[0].dy),
		);
	});
