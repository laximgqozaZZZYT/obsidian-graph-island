...
import { resolveEdgeColor, defaultColor, EDGE_TYPE_SPECS, EDGE_TYPE_FALLBACK_COLORS } from "../src/views/EdgeRenderer";
...
describe("resolveEdgeColor", () => {
	...
	it("relation color from map when useRelColor=true", () => {
		...
		const color = resolveEdgeColor(makeEdge({ relation: "Author" }), true, relColors, true);
	...
	it("inheritance uses fixed color even when useRelColor=true", () => {
		...
		const color = resolveEdgeColor(makeEdge({ type: "inheritance", relation: "is-a" }), true, relColors, true);
