...
/** Count how many non-tag nodes carry each tag (for specificity ranking). */
function countTagsAcrossNodes(nodes: readonly GraphNode[]): Map<string, number> {
...
/** Pick the rarest tag from `tags` according to `tagCounts` (smallest count wins). */
...
export function buildTagMembership(
	...
	const tagCounts = countTagsAcrossNodes(nodes);
	const tagMembership = new Map<string, Set<string>>();
	for (const n of nodes) {
		if (n.isTag || !n.tags || n.tags.length === 0) continue;
		addToMapSet(tagMembership, pickMostSpecificTag(n.tags, tagCounts), n.id);
	}
