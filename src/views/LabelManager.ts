...
export class LabelManager {
	...
	private _applyDiversityAndCap(
		...
		const zoom = this.host.getWorldScale();
		// Map-style labeling: no maxVisible cap on desktop. Mobile caps at 50 for perf.
		...
		this._applyMinFloorBoost(candidates, rt, baseOpacity, zoom, visCount);
		this._applyZoomOutEmphasis(candidates, zoom);
	}

	/** Apply priority-sorted maxVisible cap. Returns the number of visible labels. */
	...
	/** Hide a node's label, tagLabel, and subLabels. */
	 ...
	 *  candidates must be priority-desc sorted; re-enables hidden candidates
	 *  from the top until the floor is met. */
	private _applyMinFloorBoost(
		candidates: { pn: PixiNode; deg: number; isSuper: boolean; isHovered: boolean }[],
		rt: RenderThresholds,
		baseOpacity: number,
		zoom: number,
	...
	/** Zoom-out label emphasis: boost background opacity and padding for
