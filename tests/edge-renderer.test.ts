...
describe("normalizeAngle", () => {
	it("keeps angle in [0, π)", () => {
		expect(normalizeAngle(1.0)).toBeCloseTo(1.0);
	});
	it("wraps negative angle", () => {
		expect(normalizeAngle(-0.5)).toBeCloseTo(Math.PI - 0.5);
	});
	it("wraps angle >= π", () => {
		expect(normalizeAngle(Math.PI + 0.1)).toBeCloseTo(0.1);
	});
	it("normalizes 0 to 0", () => {
		expect(normalizeAngle(0)).toBeCloseTo(0);
	});
	it("normalizes π to 0", () => {
		expect(normalizeAngle(Math.PI)).toBeCloseTo(0);
	});
});
...
describe("mergeNearbyValues", () => {
	it("empty input → empty", () => {
		expect(mergeNearbyValues([], 10)).toEqual([]);
	});
	it("single value → unchanged", () => {
		expect(mergeNearbyValues([42], 10)).toEqual([42]);
	});
...
describe("deduplicatePath", () => {
	it("empty path → empty", () => {
		expect(deduplicatePath([])).toEqual([]);
	});
...
describe("angleDist", () => {
	it("zero for identical", () => {
		expect(angleDist(1, 1)).toBeCloseTo(0);
	});
	it("small distance", () => {
		expect(angleDist(0.5, 1.0)).toBeCloseTo(0.5);
	});
	it("wraps around 2π", () => {
		expect(angleDist(0.1, 2 * Math.PI - 0.1)).toBeCloseTo(0.2, 3);
	});
	it("symmetric", () => {
		expect(angleDist(1, 2)).toBeCloseTo(angleDist(2, 1));
	});
});

describe("shortestAngleDelta", () => {
	it("positive for clockwise", () => {
		expect(shortestAngleDelta(0, 1)).toBeCloseTo(1);
	});
	it("negative for counter-clockwise", () => {
...
describe("getEdgeLabel", () => {
	it("custom relation takes priority", () => {
		expect(getEdgeLabel(makeEdge({ relation: "Author" }))).toBe("Author");
	});
	it("inheritance → is-a", () => {
		expect(getEdgeLabel(makeEdge({ type: "inheritance" }))).toBe("is-a");
	});
	it("aggregation → has-a", () => {
...
describe("EDGE_TYPE_SPECS", () => {
	it("contains all known edge type constants", () => {
		const expected = [
			...
			EDGE_TYPE_INLINE_RELATION,
			"category",
			"semantic", // string-literal edge types
		];
