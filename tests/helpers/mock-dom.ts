...
function addChild(
	parent: MockEl,
	tag: string,
	opts?: { cls?: string; text?: string; value?: string; attr?: Record<string, string> },
): MockEl {
	const child = createMockEl(tag);
	if (opts?.cls) {
		child.cls = opts.cls;
		child.classList.items.push(...opts.cls.split(" "));
	}
	if (opts?.text) {
...
export function createMockEl(tag = "div"): MockEl {
	const el: MockEl = {
		...
		classList: {
			add: (c: string) => {
				if (!el.classList.items.includes(c)) el.classList.items.push(c);
			},
			remove: (c: string) => {
				el.classList.items = el.classList.items.filter((x) => x !== c);
			},
		...
		_removed: false,
		createDiv(opts) {
			return addChild(el, "div", opts);
		},
		createEl(etag, opts) {
			return addChild(el, etag, opts);
		},
		createSpan(opts) {
			return addChild(el, "span", opts);
		},
		empty() {
			el.children = [];
		},
