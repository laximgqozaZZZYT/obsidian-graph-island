...
worktreeは既にすべてクリーンアップ済みです。`git worktree prune --dry-run` も出力なし、`.git/worktrees/` ディレクトリも存在しません。

このissueは**既に解決済み**です。対象のworktreeは両方とも削除済みで、stale参照も残っていません。

分解すべき実作業がないため、タスクを1つだけ出力します：
