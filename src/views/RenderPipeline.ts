...
import {
	...
	renderMidZoomTier,
} from "./render-pipeline-helpers/lod-culling";
// Re-export LOD/culling helpers for tests and other modules.
export {
	screenToWorld,
	computeZoomFadeAlpha,
	computeLodLevel,
...
// LOD / culling pure helpers (screenToWorld, computeZoomFadeAlpha,
// computeLodLevel, computeDensityScale, computeDensityMinDist,
// generateDisplacementOffsets, quickSelect) live in
// render-pipeline-helpers/lod-culling.ts and are re-exported above.
...
export class RenderPipeline {
	...
	private _renderNodesPass(
		...
		if (isExtremeZoom) {
			renderExtremeZoomTier(g, visible, tlFilteredOut, alpha, worldScale, crc);
		} else if (isMidZoom) {
			renderMidZoomTier(g, visible, tlFilteredOut, alpha, minWorldRadius, crc);
		} else {
