...
> **2026-04-26 ratchet re-baseline (Phase R)**: EdgeRenderer.ts (2702 → 2765) and
> RenderPipeline.ts (2476 → 2657) were raised to match the actual current line
> counts after Phase Q (commit 85b9b22d) bulk-applied Prettier across the
> codebase. The reflow added ~244 lines of pure whitespace/line-break — zero
> behavioral change per the Phase Q commit message — but pushed both files past
> their previous ratchet, creating a structural conflict between the new
> `format` gate (Prettier required) and the `godobj` gate (line count). The
> new values lock in **today's post-Prettier state**. "Ratchet down only"
> still applies from here — future PRs may further reduce these limits but
> never raise them.
