...
describe("addCheckboxGroup", () => {
	it("creates a checkbox group with items", () => {
		const container = makeContainer();
		addCheckboxGroup(container as any, "Options", ["A", "B", "C"], new Set(["A", "C"]), vi.fn());
	...
	it("creates checkbox inputs", () => {
		const container = makeContainer();
		addCheckboxGroup(container as any, "Items", ["x", "y"], new Set(["x"]), vi.fn());
	...
	it("creates label elements for each checkbox", () => {
		const container = makeContainer();
		addCheckboxGroup(container as any, "Tags", ["tag1", "tag2", "tag3"], new Set(["tag1"]), vi.fn());
...
describe("addSelect", () => {
	...
	it("uses description as title when provided", () => {
		const container = makeContainer();
		addSelect(container as any, "Type", [{ value: "a", label: "A" }], "a", vi.fn(), "Select the type");
