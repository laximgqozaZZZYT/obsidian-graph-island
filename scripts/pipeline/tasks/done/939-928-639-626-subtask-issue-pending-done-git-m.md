...
元 issue が既に 1 セッション (max-turns 30) で完結する粒度のため、追加分解不要。
  単一 SUBTASK として元 issue の手順をそのまま実行する。

  手順:
  1. `Glob issues/pending/*639-626*subtask*.md` で対象 1 件を特定。
     - 0 件: `Glob issues/done/*639-626*subtask*.md` 確認、該当あれば no-op で exit 0。
     - 複数件: 中止してユーザー報告。
  2. Read で対象を開き、Edit で `status: decomposed` または `status: decomposed` 行のみを `status: done` に置換。
