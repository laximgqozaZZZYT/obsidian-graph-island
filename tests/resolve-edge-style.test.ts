...
describe("resolveEdgeStyle", () => {
	...
	it("inheritance edge uses STRUCTURAL alpha", () => {
		const s = resolveEdgeStyle(makeEdge({ type: "inheritance" }), src, tgt, makeCfg(), 1, null);
		expect(s.alpha).toBeCloseTo(STRUCTURAL_EDGE_ALPHA);
	});

	it("has-tag edge is structural", () => {
		const s = resolveEdgeStyle(makeEdge({ type: "has-tag" }), src, tgt, makeCfg(), 1, null);
		expect(s.alpha).toBeCloseTo(STRUCTURAL_EDGE_ALPHA);
	});

	it("similar edge is structural", () => {
		const s = resolveEdgeStyle(makeEdge({ type: "similar" }), src, tgt, makeCfg(), 1, null);
		expect(s.alpha).toBeCloseTo(STRUCTURAL_EDGE_ALPHA);
	});
	...
	it("highContrast doubles lineThick and boosts alpha", () => {
		...
		const hc = resolveEdgeStyle(makeEdge(), src, tgt, makeCfg({ highContrast: true }), 1, null);
