...
/** Pass 1 of buildTagMembership: count how many non-tag nodes carry each tag. */
function _countTagOccurrences(nodes: readonly GraphNode[]): Map<string, number> {
...
/** Pick the rarest (smallest-count) tag from a node's tag list. */
function _pickMostSpecificTag(tags: readonly string[], tagCounts: Map<string, number>): string {
	let bestTag = tags[0];
...
/** Extract bidirectional tag relationship pairs from inheritance/aggregation edges. */
function _extractTagRelPairs(edges: readonly GraphEdge[]): Set<string> {
...
export function buildTagMembership(
	...
	const tagCounts = _countTagOccurrences(nodes);
	const tagMembership = new Map<string, Set<string>>();
	for (const n of nodes) {
		if (n.isTag || !n.tags || n.tags.length === 0) continue;
		const bestTag = _pickMostSpecificTag(n.tags, tagCounts);
		addToMapSet(tagMembership, bestTag, n.id);
	}
	const tagRelPairs = _extractTagRelPairs(edges);
