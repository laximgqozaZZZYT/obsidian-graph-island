...
function computeComparison(
  ...
  return {
    sharedNeighbors, uniqueToA, uniqueToB,
    sharedTags, uniqueTagsA, uniqueTagsB,
    sharedCategories, shortestPath, pathLength,
  };
}
...
describe("bfs (shortest path)", () => {
  ...
  it("finds shortest path when multiple paths exist", () => {
    ...
    const adj = makeAdj([["a", "b"], ["b", "d"], ["a", "c"], ["c", "d"]]);
...
describe("computeComparison", () => {
  ...
  it("finds unique neighbors for each node", () => {
    const adj = makeAdj([["a", "x"], ["b", "y"], ["a", "c"], ["b", "c"]]);
    const result = computeComparison({ id: "a" }, { id: "b" }, adj);
