...
2. **CLAUDE.md の Max Allowed は決して増加させない**（Ratchet down only ポリシー）。
   - 測定値 > 8597 の場合: CLAUDE.md を触らず、先に `GraphViewContainer.ts` から snapshot/export/filter orchestration のいずれかを別ファイル（例: `src/views/snapshot.ts`）に抽出して 8597 以下に戻す
   - 測定値 ≤ 8597 の場合のみ: CLAUDE.md の GOD OBJECT Policy テーブルの `src/views/GraphViewContainer.ts` 行の Lines / Max Allowed を測定値に更新
...
- [ ] `wc -l src/views/GraphViewContainer.ts` の出力が CLAUDE.md の該当行の Lines 値と一致
