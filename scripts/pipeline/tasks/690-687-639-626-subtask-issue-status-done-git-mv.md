...
1. Glob `issues/pending/*639-626*subtask*.md` で対象ファイル特定。
  ...
  2. Read で対象ファイル全体を取得し、Edit で frontmatter の `status: in-progress`(または `in-progress`) を `status: done` に変更。他フィールド (priority/reported/parent/depends/summary/source) と本文は一切触らない。
