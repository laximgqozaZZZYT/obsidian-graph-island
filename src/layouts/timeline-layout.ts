...
export function timelinePartitionNodes(
	...
	hierChildrenMap?: Map<string, string[]>;
} {
	const getNodeProperty = cfg.getNodeProperty;
	...
	const orderFields = orderFieldStr
		.split(",")
		.map((f) => f.trim())
		.filter(Boolean);
	...
	if (untimed.length > 0 && getNodeProperty) {
		if (hasSequenceFields) {
			const { order: chainOrder, chains } = buildLinkChainOrder(
				untimed,
				getNodeProperty,
				fwdSeqFields,
				revSeqFields,
...
export function timelineSortAndBuildSteps(timed: { node: GraphNode; value: string }[]): {
...
export function timelineComputeSpacing(
	...
	const maxCols = 40;
	const minSpacing = totalCols > 200 ? nodeSize * 1.2 : totalCols > 100 ? nodeSize * 1.8 : nodeSize * 3;
	const effectiveSpacing = totalCols > maxCols ? Math.max(minSpacing, (spacing * maxCols) / totalCols) : spacing;
...
export function timelineCenterOffsets(offsets: Map<string, { dx: number; dy: number }>): {
	xCenter: number;
	yCenter: number;
} {
	let minX = Infinity,
		...
		if (endTimeIdx !== undefined) {
			...
			if (allNumeric && !isNaN(Number(endVal))) {
				...
				for (let i = 0; i < uniqueTimes.length; i++) {
					if (Number(uniqueTimes[i]) >= endNum) {
						bestIdx = i;
						break;
					}
				}
...
export function timelineRecenterY(offsets: Map<string, { dx: number; dy: number }>, bars: TimelineBarInfo[]): void {
	let minY2 = Infinity,
		maxY2 = -Infinity;
...
export function timelineBuildSequenceEdges(sortedTimed: { node: GraphNode; value: string }[]): GraphEdge[] {
...
export function buildHierarchyOrder(
	...
	const order = new Map<string, number>() as Map<string, number> & {
		parentMap?: Map<string, string>;
		childrenMap?: Map<string, string[]>;
	};
	const idSet = new Set(members.map((n) => n.id));
