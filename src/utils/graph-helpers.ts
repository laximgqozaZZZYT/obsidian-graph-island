...
/** Pass 1: count members per tag (used for specificity ranking). */
...
/** Pick the tag with the smallest member count (most specific). */
...
/** Pass 2: assign each non-tag node to its most specific tag bucket. */
...
/** Pass 3: collect tag↔tag relationship pairs from inheritance/aggregation edges. */
function collectTagRelPairs(edges: readonly GraphEdge[]): Set<string> {
...
export function buildTagMembership(
	...
	const tagRelPairs = collectTagRelPairs(edges);
