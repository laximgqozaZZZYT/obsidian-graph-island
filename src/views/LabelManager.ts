...
export class LabelManager {
	...
	private _applyDiversityAndCap(
		...
		// Mobile lightweight mode caps labels to 50 for performance.
		...
		// Sort by priority score (desc) before cap and floor
	...
	/** Cap pass: show top labels up to maxVisible (super-/hovered nodes bypass). */
	private _applyMaxVisibleCap(
		...
		const showAlpha = Math.max(minAlpha, baseOpacity);
		let visCount = 0;
		for (const c of candidates) {
			const { pn, isHovered, isSuper } = c;
			if (isHovered || isSuper) {
				this._showLabel(pn, showAlpha);
				continue;
			}
	...
	/** Floor pass: at extreme zoom-out, re-enable hidden non-super labels until
	 *  the visible count reaches `labelMinVisibleFloor`. */
	private _applyPriorityFloor(
		...
		if (minFloor <= 0 || hardHideZoom <= 0) return;
		if (zoom >= hardHideZoom || visCount >= minFloor) return;
	...
	/** Zoom-out emphasis: boost background opacity/padding for surviving labels. */
	...
	/** Make a node's primary label visible at the given alpha. */
	...
	/** Hide a node's primary label and any companion labels (tag/sub). */
	private _hideLabel(pn: PixiNode): void {
		pn.label!.visible = false;
		pn.label!.alpha = 0;
		pn.labelWasVisible = false;
		if (pn.tagLabel) pn.tagLabel.visible = false;
