...
describe("timelinePlaceUntimedNodes", () => {
	...
	it("sorts by label for deterministic ordering", () => {
		const nodes = [{ ...mkNode("z"), label: "zzz" } as GraphNode, { ...mkNode("a"), label: "aaa" } as GraphNode];
		const offsets = new Map<string, { dx: number; dy: number }>();
...
describe("timelineAssignBarLanes", () => {
	...
	it("applies compact scaling when total lanes exceed target height", () => {
		...
		const bars = Array.from(
			{ length: 50 },
			(_, i) => mkBar(`n${i}`, 0, 100), // all overlap → 50 lanes
		);
...
describe("timelineRecenterY", () => {
	...
	it("also adjusts bar yCenter values", () => {
		...
		const bars: TimelineBarInfo[] = [{ nodeId: "a", xStart: 0, xEnd: 50, barHeight: 30, yCenter: 100 }];
