...
1. Glob `issues/pending/*639-626*subtask*.md` で対象を特定。
  ...
  2. Read で frontmatter 確認 → Edit で `status: in-progress` または `status: pending` を `status: done` に書換。他フィールド (priority/reported/parent/depends/summary/source) と本文は変更禁止。
