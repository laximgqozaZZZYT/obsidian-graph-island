...
(globalThis as any).ClipboardItem = class { constructor(public items: any) {} };
...
describe("exportSubgraph", () => {
  ...
  it("uses hoverHops from panel", () => {
    ...
    expect(vi.mocked(collectSubgraph)).toHaveBeenCalledWith(
      expect.anything(), "node-a", 3, expect.anything(), expect.anything()
    );
  });
  ...
  it("defaults hoverHops to 2 when not set", () => {
    ...
    exportSubgraph(host, "node-a");
    expect(vi.mocked(collectSubgraph)).toHaveBeenCalledWith(
      expect.anything(), "node-a", 2, expect.anything(), expect.anything()
    );
  });
