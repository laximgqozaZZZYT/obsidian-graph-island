...
function converge(force: (alpha: number) => void, iterations = 60) {
...
describe("buildClusterForce", () => {
	...
	it("moves nodes toward target positions", () => {
		const nodes = [makeNode("a", { tags: ["t1"], x: 0, y: 0 }), makeNode("b", { tags: ["t1"], x: 0, y: 0 })];
	...
	it("kills velocity completely each tick", () => {
		const nodes = [makeNode("a", { tags: ["t1"], vx: 100, vy: 200 })];
		const force = extractForce(buildClusterForce(nodes, [], new Map(), baseCfg()))!;
		force(1);
	...
	it("converges to stable positions after many iterations", () => {
		const nodes = [makeNode("a", { tags: ["t1"], x: 0, y: 0 }), makeNode("b", { tags: ["t1"], x: 500, y: 500 })];
		const force = extractForce(buildClusterForce(nodes, [], new Map(), baseCfg()))!;
		converge(force);
...
describe("concentric arrangement", () => {
	...
	it("larger group produces larger outer radius", () => {
		...
		const smallForce = extractForce(
			buildClusterForce(smallNodes, [], new Map(), baseCfg({ arrangement: "concentric" })),
		)!;
		converge(smallForce);
		...
		const largeForce = extractForce(
			buildClusterForce(largeNodes, [], new Map(), baseCfg({ arrangement: "concentric" })),
		)!;
		converge(largeForce);
...
describe("grid arrangement", () => {
	it("arranges nodes in a square grid pattern", () => {
		...
		const force = extractForce(buildClusterForce(nodes, [], new Map(), baseCfg({ arrangement: "grid" })))!;
		converge(force);
...
describe("group separation", () => {
	...
	it("groups by backlinks bucket separate correctly", () => {
		...
		const force = extractForce(
			buildClusterForce(
				nodes,
				[],
				degrees,
				baseCfg({ groupRules: [{ groupBy: "backlinks", recursive: false }], arrangement: "grid" }),
			),
		)!;
		converge(force);
	...
	it("groups by node_type separate tag nodes from file nodes", () => {
		...
		const force = extractForce(
			buildClusterForce(
				...
				[],
				new Map(),
				baseCfg({ groupRules: [{ groupBy: "node_type", recursive: false }], arrangement: "concentric" }),
			),
		)!;
		converge(force);
...
describe("large-scale layout", () => {
	...
	it("no two nodes in the same group overlap exactly", () => {
		...
		const force = extractForce(buildClusterForce(nodes, [], new Map(), baseCfg({ arrangement: "concentric" })))!;
		converge(force);
...
describe("d3 simulation pipeline integration", () => {
	...
	function createSimWithCluster(
		...
		const sim = forceSimulation(nodes).alphaDecay(0.08).velocityDecay(0.55).stop();
...
describe("multi-level grouping", () => {
	...
	it("recursive flag per rule splits connected components independently", () => {
		...
		const fResult = buildClusterForce(
			nodes,
			edges,
			new Map(),
			baseCfg({
	...
	it("tag then backlinks: produces hierarchical grouping", () => {
		...
		const fResult2 = buildClusterForce(
			nodes,
			[],
			degrees,
			baseCfg({
...
describe("estimateLabelExtent", () => {
	const mkN = (label: string, opts?: Partial<GraphNode>): GraphNode => ({ id: label, label, ...opts }) as GraphNode;
	...
	it("super node uses different padding", () => {
		...
		const superNode = estimateLabelExtent(mkN("test", { collapsedMembers: ["a", "b"] as any }), 10, 5, 10, 1);
...
describe("analyzeOverlap", () => {
	it("returns zeros for single node", () => {
		const result = analyzeOverlap([{ id: "a", x: 0, y: 0 }], new Map([["a", 10]]), 3);
