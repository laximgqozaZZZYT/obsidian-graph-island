...
export class NodeDetailView extends ItemView {
	...
	private renderRelationDrawer(parent: HTMLElement, nodeId: string, edges: GraphEdge[], pixiNodes: Map<string, any>) {
		...
		for (const [type, items] of grouped) {
			this.renderCollapsibleList(parent, `${type} (${items.length})`, items, (item) => {
	...
	private async renderPreview(parent: HTMLElement, file: TFile) {
		...
		await MarkdownRenderer.render(this.app, preview, previewContent, file.path, this.renderComponent!);
	}
	...
	private async renderInlinePreview(container: HTMLElement, filePath: string) {
		...
		await MarkdownRenderer.render(this.app, preview, container, tf.path, this.renderComponent!);
	}
	...
	private renderProperties(parent: HTMLElement, file: TFile) {
		...
		details.createEl("summary", {
			...
			text: `${t("detail.properties")} (${entries.length})`,
		});
	...
	private renderBacklinks(parent: HTMLElement, file: TFile) {
		...
		this.renderCollapsibleList(parent, `${t("detail.backlinks")} (${backlinks.length})`, backlinks, (bl) => ({
			label: bl.basename,
			isTag: false,
			filePath: bl.path,
	...
	private renderLinkSuggestions(
		...
		details.createEl("summary", {
			cls: "gi-detail-section-label",
			text: `${t("detail.suggestedLinks")} (${suggestions.length})`,
		});
