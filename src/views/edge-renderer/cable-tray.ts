/**
 * cable-tray drawing helpers — extracted from EdgeRenderer.ts.
 *
 * These functions perform the actual canvas drawing for the cable-tray
 * wiring model.  They take a CanvasGraphics target as a parameter rather
 * than holding it in module state, so each function is fully testable in
 * isolation.  Pure layout / route computation continues to live in
 * CableTrayRenderer.ts; this file is only the rendering pass.
 */
 ...
 */
export function drawSingleIntraCableBranches(
	g: CanvasGraphics,
 ...
 */
export function drawSingleIntraCableGpb(
	g: CanvasGraphics,
	cable: IntraGroupCable,
	cfg: EdgeDrawConfig,
	densityScale: number,
...
export function drawIntraGroupCables(
	...
	const zoomFade = zoomFadeAlpha(ws);
	...
	const drawBranchWires = (filterHighlight: "normal" | "bright" | "dim" | null) => {
	...
	if (cfg.highlightedNodeId) {
		drawBranchWires("dim");
		drawBranchWires("bright");
	} else {
		drawBranchWires(null);
	}

	// PASS 2: Single group port branch wires (1 port per group).
	if (cfg.highlightedNodeId) {
		const drawGpbWires = (filterHL: "dim" | "bright") => {
			for (const cable of cables) {
...
export function splitHighlightEdges(
	wireEdges: GraphEdge[],
...
export function drawHighlightedWire(
	g: CanvasGraphics,
 ...
 */
export function drawSingleTrunk(
	g: CanvasGraphics,
	trunk: Trunk,
	cfg: EdgeDrawConfig,
	densityScale: number,
	...
	for (let ci = 0; ci < nUnique; ci++) {
		...
		const buildTrunkWirePath = (): { x: number; y: number }[] => {
		...
		if (cfg.highlightedNodeId) {
			...
			drawSmoothPath(g, buildTrunkWirePath(), wireWidth, color, wireAlpha);
		}
	}
}
 ...
 * Uses drawSmoothPath for all rendering.
 */
export function drawTrunks(
	...
	const drawTrunkWires = (filterHighlight: "bright" | "dim" | "normal" | null) => {
		for (const trunk of trunks) {
