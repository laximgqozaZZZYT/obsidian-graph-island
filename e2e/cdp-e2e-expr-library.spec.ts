/**
 * CDP E2E Test -- Expression Library
 *
 * Verifies that expression library entries exist and produce
 * distinct node distributions when applied.
 */
...
test("grid expression produces rectangular distribution", async () => {
  const result = await page.evaluate(async () => {
    ...
    return { nodeCount: xs.length, xRange: Math.max(...xs) - Math.min(...xs), yRange: Math.max(...ys) - Math.min(...ys) };
  });
...
test("spiral expression produces radial distribution", async () => {
  const result = await page.evaluate(async () => {
    ...
    return { nodeCount: dists.length, maxDist: Math.max(...dists) };
  });
  expect(result).not.toHaveProperty("error");
