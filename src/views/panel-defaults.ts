 ...
 *
 * Each factory body uses `satisfies Partial<PanelState>` so that the
 * object literal receives PanelState's contextual types — literal
 * unions narrow without `as Type` casts and arrays/objects are typed
 * via the parent property's declared type. The spread in
 * createDefaultPanelState() then composes the four narrow slices
 * into a fully-typed PanelState.
 */
 ...
 *
 *  The four DEFAULT_* factories partition PanelState's required keys
 *  exactly (verified by tests/views/panel-defaults.test.ts), so the
 *  combined spread structurally satisfies PanelState. The final
 *  assertion is needed because each factory's individually narrow
 *  type does not formally widen back to PanelState during inference. */
