...
describe("Viewport margin cap", () => {
	const vpMargin = 100;
	const computeEffectiveMargin = (zoom: number) => Math.min(vpMargin / zoom, vpMargin * 5);
...
describe("Card scale cap", () => {
	const computeCardScale = (worldScale: number) => Math.min(1 / worldScale, CARD_SCALE_CAP);
